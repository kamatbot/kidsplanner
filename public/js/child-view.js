...
(function () {
  ...
  function hero(id, date, data, state, sources) {
    ...
    return `<section class="cv-panel cv-hero" aria-label="${e(label)}"><div class="cv-hero-rings">${rings}</div><div class="cv-hero-stats"><div class="cv-metric cv-metric-hw"><strong>${hwState}</strong><span>${metric.homeworkKnown ? metric.hwLeft ? 'Homework left this week' : 'No homework left this week' : 'Homework unavailable'}</span></div><div class="cv-metric cv-metric-d3"><strong>${d3State}</strong><span>${metric.dailyComplete == null ? 'Daily 4 unavailable' : 'Daily 4 today'}</span></div><div class="cv-metric cv-metric-hab"><strong>${habitState}</strong><span>${metric.habitsKnown ? metric.habits.length ? 'Habits today' : 'No habits yet' : 'Habits unavailable'}</span></div></div></section><section class="cv-panel cv-weekly"><h2>This week</h2>${weeklyBars('Homework', metric.days, metric.homeworkDays, 'var(--fr-hw)')}${dailyWeek}${weeklyBars('Habits', metric.days, metric.habitDays, 'var(--fr-hab)')}<p class="cv-week-note">Unrecorded days remain unfilled.</p></section>`;
  }
