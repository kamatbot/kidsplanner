  ...
  return `<article class="fr-kid-card" data-kid-id="${esc(kid.id)}">
    ...
    <div class="fr-kid-body"><button type="button" class="fr-kid-rings" onclick="${open}" aria-label="${parent ? `Open ${name}'s progress` : 'Open your homework'}">${famRing({ rings, key: `kid-${kid.id}`, label: summary })}</button>
      <div class="fr-kid-stats">
        <button type="button" class="fr-stat fr-stat-hw" onclick="openTodayKidHomework('${id}')"><b>${hwKnown ? hw.left : '—'}</b><span>${hwKnown ? `Homework left this week${hw.total && !hw.left ? ' <span class="fr-stat-note">· Done ✓</span>' : ''}` : hwUnknown}</span></button>
        <button type="button" class="fr-stat fr-stat-d3" onclick="openTodayLearning()"><b>${daily3 ? `${daily3.done}/4` : '—'}</b><span>Daily 4 today <span class="fr-stat-note">· ${daily3 ? esc(daily3.status) : progressState === 'loading' ? 'Loading…' : 'Unavailable'}</span></span></button>
        ${!habKnown ? `<div class="fr-stat fr-stat-hab"><b>—</b><span>${habUnknown}</span></div>` : hab.total ? `<details class="fr-habit-details"><summary class="fr-stat fr-stat-hab"><b>${hab.done}/${hab.total}</b><span>Habits today <span class="fr-stat-note">· ${hab.done === hab.total ? 'Done ✓' : 'Check in'}</span></span></summary><div class="fr-habit-popover"><h3>${name}’s habits</h3>${habits.map(renderTodayHabitRow).join('')}<button class="today-link" type="button" onclick="switchNavTab('goals')">All goals ${todayIcon('arrow')}</button></div></details>` : `<button type="button" class="fr-stat fr-stat-hab" onclick="switchNavTab('goals')"><b>—</b><span>No habits yet <span class="fr-stat-note">· <em>Set a first habit</em></span></span></button>`}
      </div>
    </div>
