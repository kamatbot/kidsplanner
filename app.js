...
function getUsers()    { return load('kp_users')    || []; }
function saveUsers(u)  { save('kp_users', u); }
function getEvents()   { return load('kp_events')   || []; }
function saveEvents(e) { save('kp_events', e); }
function getSchedules(){ return load('kp_schedules')|| []; }
function saveSched(s)  { save('kp_schedules', s); }
...
function seedDemo() {
  ...
  if (!users.find(u => u.email === 'kid@demo.com')) {
    users.push({ id: 'demo-kid',    name: 'Alex',        email: 'kid@demo.com',    password: 'demo123', role: 'kid',    grade: '7th Grade' });
    users.push({ id: 'demo-parent', name: 'Sam (Parent)', email: 'parent@demo.com', password: 'demo123', role: 'parent', grade: '' });
    saveUsers(users);
  }
  ...
  const seed = [
    { id: 'se1', userId: 'demo-kid', title: 'Math Class',           date: isoDate(today),                                  time: '09:00', endTime: '10:00', category: 'school', notes: 'Chapter 5 test today!' },
    { id: 'se2', userId: 'demo-kid', title: 'Soccer Practice',       date: isoDate(new Date(+today + 86400000)),            time: '15:30', endTime: '17:00', category: 'sports', notes: 'Bring shin guards!' },
    { id: 'se3', userId: 'demo-kid', title: 'Science Project Due',   date: isoDate(new Date(+today + 2 * 86400000)),        time: '',      endTime: '',      category: 'school', notes: 'Solar system model' },
    { id: 'se4', userId: 'demo-kid', title: 'Art Club',              date: isoDate(new Date(+today + 3 * 86400000)),        time: '13:00', endTime: '14:30', category: 'arts',   notes: 'Pottery session' },
    { id: 'se5', userId: 'demo-kid', title: "Emma's Birthday Party", date: isoDate(new Date(+today + 5 * 86400000)),        time: '14:00', endTime: '17:00', category: 'social', notes: 'Bring a gift 🎁' },
  ];
  seed.forEach(e => { if (!events.find(ev => ev.id === e.id)) events.push(e); });
  saveEvents(events);
}
...
/* ============================================================
   AUTH
============================================================ */
function toggleGradeField(show) {
  document.getElementById('grade-group').style.display = show ? 'block' : 'none';
}
...
function handleSignup(e) {
  ...
  const user = { id: uid(), name, email, password: pwd, role, grade };
  users.push(user);
  saveUsers(users);
  loginUser(user);
}
...
function showDashboard() {
  ...
  document.getElementById('header-greeting').textContent = `${greet}, ${currentUser.name}! 👋`;
  ...
  const roleLabel = currentUser.role === 'parent'
    ? '👨‍👩‍👧 Parent'
    : `🧒 Kid${currentUser.grade ? ' · ' + currentUser.grade : ''}`;
...
function showDetail(id) {
  ...
  document.getElementById('btn-delete-event').style.display = currentUser.role === 'parent' ? '' : 'none';
...
function renderUploads() {
  ...
  const mine = getSchedules().filter(s => s.userId === currentUser.id);
...
function renderWidgets() {
  ...
  const offset  = load(`kp_qoffset_${currentUser.id}`) || 0;
...
function nextQuestion() {
  const key    = `kp_qoffset_${currentUser.id}`;
...
function renderStreak() {
  const key    = `kp_streak_${currentUser.id}`;
