...
- **Action stack:** each row has a lead (a completion ring with a `--text-2` border and a 44px hit area, or the child's avatar for parent homework reviews), a title, a middot metadata line, and snooze as an icon-only button. The first row is the lead item: 17px, with a violet ring or a primary Review button. Completing an action fills the circle green, strikes the title and collapses the row in 280ms: the one authored motion on the page.
- **Schedule timeline:** a mono time column, a 1px rail with nodes in each child's colour (neutral for family events, hollow once past), titles at 15px, and a coral "Now" line. Once the day is over, the timeline shows tomorrow instead. Dinner rows show a bowl icon instead of the feed's emoji.
