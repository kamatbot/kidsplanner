...
| Kid sign-in (decided 2026-07-03) | **Parent-provisioned passkey per device.** A kid has NO email and NO self-signup. To give a kid access on their own iPhone/iPad, the parent — authenticated on that device — runs an "add [kid]'s device" flow that enrolls a device-bound passkey for that kid profile. The kid then signs in with Face ID / Touch ID. Kid sessions are role-scoped: their own calendar/homework/goals + family chat, but NOT billing, family management (add/remove parents or kids), or removing members. Registration is always parent-initiated (consent preserved); the kid credential is bound to that device. Net-new work — see build plan. |
