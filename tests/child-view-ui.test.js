...
test('synced Daily 4 without completions shows not done; failed sources identify recovery', async () => {
  const { nodes, view } = setup({ getGoals: async () => { throw new Error('offline'); } }); await view.render('one');
  const html = nodes['tab-child'].innerHTML;
  assert.match(html, /0 of 4 done/); assert.equal((html.match(/>Not done</g) || []).length, 4); assert.match(html, /Habits unavailable/); assert.match(html, /id="cv-fams"/);
...
test('habit counts use exactly the seven displayed local dates; Daily 4 ignores invalid timestamps', async () => {
  ...
  await view.render('one'); const html = nodes['tab-child'].innerHTML;
  assert.match(html, /2 of 7 days recorded/); assert.match(html, /1 of 4 done/); assert.equal((html.match(/>Done</g) || []).length, 1); assert.equal((html.match(/>Not done</g) || []).length, 3); assert.match(html, /2026-09-02: checked in/); assert.doesNotMatch(html, /2026-09-01: checked in/);
});
