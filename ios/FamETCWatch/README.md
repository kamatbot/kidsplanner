...
The standalone watch test suite passed 32 tests; the subsequent focused store/complication run passed 20 tests, including two added regressions (34 unique tests in total). The iPhone app with its embedded parent watch and widget also builds successfully with simulator signing. Synthetic HTTP checks verified child code claim, own/shared calendar scope, privileged endpoint denial, and revoked credentials.
