...
- **Deploy flow** — `scripts/pack-deploy.sh` is the canonical Hostinger packer; `scripts/deploy-hostinger.sh` is a deprecated compatibility wrapper that delegates to it (preserving the legacy output path/default Builds archive and Node 24 guard). See `CLAUDE.md` and `docs/CI-RELEASE.md` for the current release contract.
