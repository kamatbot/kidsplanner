...
function extractTravelResults(text) {
  ...
  if (matches.length !== 1) return { text: source, card: null };
  let raw;
  try { raw = JSON.parse(matches[0][1]); } catch (_) { return { text: source.replace(matches[0][0], "").trim(), card: null }; }
  const card = travelResults.sanitizeTravelCard(raw);
  const visible = source.replace(matches[0][0], "").trim();
  return {
    text: visible || (card ? "I found a few current options for the trip." : source.trim()),
    card,
  };
}
function sendAgentMessage(scopeKey, text, card) {
  ...
  const inferredCard = isFamilyScope(scopeKey) && mealPlanImport.isParseableMealPlan(visibleText)
    ...
    : trip && tripItineraryImport.isParseableItinerary(visibleText, trip) ? TRIP_ITINERARY_DRAFT_CARD : structuredTravelCard;
  let messageCard;
  if (card !== undefined) {
