...
const MEAL_PLAN_DRAFT_CARD = Object.freeze({
  type: "meal-plan-draft",
  id: "hermes-meal-plan",
  title: "Meal plan ready",
});

const TRIP_ITINERARY_DRAFT_CARD = Object.freeze({
  type: "trip-itinerary-draft",
  id: "hermes-trip-itinerary",
  title: "Itinerary ready",
});

function isFamilyScope(scopeKey) {
  return typeof scopeKey === "string" && !scopeKey.startsWith(TRIP_SCOPE_PREFIX);
}

function sendAgentMessage(scopeKey, text, card) {
  const trip = typeof scopeKey === "string" && scopeKey.startsWith(TRIP_SCOPE_PREFIX)
    ? trips.getTrip(scopeKey.slice(TRIP_SCOPE_PREFIX.length))
    : null;
  const inferredCard = isFamilyScope(scopeKey) && mealPlanImport.isParseableMealPlan(text)
    ? MEAL_PLAN_DRAFT_CARD
    : trip && tripItineraryImport.isParseableItinerary(text, trip)
      ? TRIP_ITINERARY_DRAFT_CARD
      : null;
