...
function roomForFamily(fam, roomId) {
  ...
  if (hermesThreads.isHermesScope(roomId)) {
    // A member's private thread: bridge room id and scope key are identical.
    const member = hermesThreads.memberFor(fam, roomId.slice(hermesThreads.SCOPE_PREFIX.length));
    if (!member) return null;
    return { roomId, scopeKey: roomId, kind: "assistant", title: `Hermes · ${member.name}`, member };
  }
...
function roomsForFamily(fam) {
  ...
  for (const member of hermesThreads.membersOf(fam)) {
    rooms.push({ roomId: hermesThreads.scopeFor(member.userId), title: `Hermes · ${member.name}`, kind: "assistant" });
  }
  return rooms;
}
...
// Shared rooms forward only explicit @Hermes mentions; in a member's private
// Hermes thread every message is addressed to Hermes.
function isInboundMessage(message, scopeKey) {
  return !!message && HUMAN_SENDER_TYPES.has(message.senderType) && !message.deleted
    && typeof message.text === "string" && !!message.text.trim()
    && (hermesThreads.isHermesScope(scopeKey) || isHermesMention(message.text));
}
...
function listInboundMessages(scopeKey, afterId) {
  ...
  const project = (all) => all.filter((message) => isInboundMessage(message, scopeKey)).map((message) => withTripContext(scopeKey, message));
...
// Meal-plan draft cards import from the family room only (lib/routes/meals.js).
function isFamilyScope(scopeKey) { return typeof scopeKey === "string" && !scopeKey.startsWith(TRIP_SCOPE_PREFIX) && !hermesThreads.isHermesScope(scopeKey); }
