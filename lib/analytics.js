"use strict";
/**
 * Fam ETC — tiny, privacy-first analytics engine (ported from RetireOdds).
 *
 ...
 *   - events     allowlisted app events (app_open, onboarding_complete,
 *                action_created/completed/snoozed, …)
 *
...
function summary(days = 30) {
  ...
  for (const date of dates) {
    ...
    for (const [k, v] of Object.entries(r.events || {})) {
      if (!ALLOWED_EVENTS.has(k)) continue;
      bump(eventTotals, k, v);
      safeEvents[k] = v;
    }
    series.push({
      date,
  ...
  return { days: n, range: { from: dates[0], to: dates[dates.length - 1] }, totals, events: eventTotals, conversion, platforms, topPaths, series, experiments };
}
