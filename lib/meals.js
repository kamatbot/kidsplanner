...
const MENU_SOURCE_TYPES = new Set(["chat"]);
const MENU_SOURCE_ID_RE = /^m_[A-Za-z0-9_-]+$/;
const MAX_MENU_SOURCE_ID_LENGTH = 200;
...
function getMenuEntriesBySource(familyId, sourceType, sourceId) {
  if (!sourceType || !sourceId) return [];
  return famMeals(familyId).menu.filter((e) => e.source === "hermes" && e.sourceType === sourceType && e.sourceId === sourceId);
}
...
function addMenuEntry(familyId, userId, opts = {}) {
  ...
  const source = opts.source === "hermes" ? "hermes" : (opts.source === "ai" ? "ai" : "manual");
  const hasSourceType = opts.sourceType !== undefined && opts.sourceType !== null;
  const hasSourceId = opts.sourceId !== undefined && opts.sourceId !== null;
  let menuSource = null;
  if (source === "hermes") {
    if (!hasSourceType || !hasSourceId || !MENU_SOURCE_TYPES.has(opts.sourceType)
        || typeof opts.sourceId !== "string" || opts.sourceId.length > MAX_MENU_SOURCE_ID_LENGTH
        || !MENU_SOURCE_ID_RE.test(opts.sourceId)) {
      return { error: "A valid Hermes chat source is required." };
    }
    menuSource = { sourceType: opts.sourceType, sourceId: opts.sourceId };
  }
  ...
  if (menuSource) Object.assign(entry, menuSource);
  m.menu.push(entry);
  db.persist();
  return { entry };
}
...
function addHermesMenuEntry(familyId, userId, opts = {}) {
  return addMenuEntry(familyId, userId, Object.assign({}, opts, {
    source: "hermes",
    sourceType: "chat",
  }));
}
...
function buildHousehold(fam, resolveParentProfile) {
  ...
  for (const kid of (fam && fam.kids) || []) {
    ...
    members.push({ kidId: kid.id, name: kid.name, kind: "kid", color: kid.color, photo: kid.photo || "", portion, allergies });
    totalFactor += PORTION_FACTORS[portion];
  }
