...
function createOperatorExecution(options = {}) {
  ...
  function validateApprovedAction(familyId, row) {
    ...
    if (!driver) {
      throw new OperatorExecutionError(
        `Action type ${row.action_type} has no approved execution driver.`,
  ...
  function claimExecution(familyId, approvalId, input = {}) {
    ...
    if ((row.state === "claimed" || row.state === "running") && !tokenExpired) {
    ...
    const tx = db.transaction(() => {
      db.prepare(`
        ...
        WHERE id = ?
      `).run(executorType, tokenHash(executionToken), expiresAt, claimedAt, claimedAt, row.id);
      insertAudit(db, {
        familyId,
        caseId: row.case_id,
        actorId: actor.userId,
  ...
  function runExecution(familyId, executionToken, actionHash, input = {}) {
    ...
    if (row.state === "claimed") {
      const txRunning = db.transaction(() => {
        const update = db.prepare(`
          UPDATE operator_execution_grants
