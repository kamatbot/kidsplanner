...
function validateActionUpdate(familyId, action) {
  ...
  // Operator writes are parent-approved; approval cannot grant student-only
  // homework progress permissions through its derived Today action.
  if (current.sourceType === "homework" && ["status", "snoozedUntil"].some((key) => Object.prototype.hasOwnProperty.call(patch, key))) {
    throw new OperatorExecutionError("Homework progress can only be changed by the student.", "EXECUTION_ACTION_INVALID");
  }
...
function createOperatorExecution(options = {}) {
  ...
  function listContinuations(familyId) {
    return requireDb().prepare(`
      SELECT g.approval_id AS approvalId, a.decided_by AS parentUserId
      FROM operator_execution_grants g
      JOIN operator_approvals a ON a.id = g.approval_id
      JOIN operator_cases c ON c.id = g.case_id AND c.family_id = g.family_id
      WHERE g.family_id = ? AND a.state = 'approved'
        AND g.action_type IN ('calendar.create', 'action.create')
        AND c.state IN ('executing', 'verifying')
        AND (g.state IN ('ready', 'consumed') OR
          (g.state IN ('claimed', 'running') AND g.token_expires_at <= ?))
      ORDER BY CASE WHEN g.state = 'consumed' AND EXISTS (
        SELECT 1 FROM operator_case_steps s WHERE s.case_id = g.case_id
          AND s.idempotency_key = 'continuation:' || g.id || ':verified'
          AND s.state = 'completed'
      ) THEN 1 ELSE 0 END, g.updated_at ASC LIMIT 20
    `).all(familyId, nowIso());
  }
  function close() { if (!database) return; try { database.close(); } finally { database = null; initAttempted = true; } }
  return { status, supportedActionTypes, validateAction, listApprovalsForParent, getApprovalForParent, decideApproval, claimExecution, runExecution, getExecutionForApproval, listContinuations, close };
}
...
module.exports = {
  ...
  listContinuations: (...args) => defaultExecution().listContinuations(...args),
