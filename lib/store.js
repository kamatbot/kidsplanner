...
// Signed cookie-session cookies can be replayed until the browser-side expiry
// unless the server has a revocation fact of its own. Keep a monotonic session
// generation on each user. Sessions minted before this field existed are
// generation 0, so rollout does not force every existing device to sign in
// again. Logout bumps the generation; every copied/older cookie then fails on
// the next request even though its signature remains cryptographically valid.
function sessionGeneration(userOrId) {
  const user = typeof userOrId === "string" ? getUser(userOrId) : userOrId;
  if (!user) return null;
  const n = Number(user.authSessionGeneration);
  return Number.isSafeInteger(n) && n >= 0 ? n : 0;
}

function bumpSessionGeneration(userId) {
  const user = getUser(userId);
  if (!user) return null;
  const next = (sessionGeneration(user) || 0) + 1;
  user.authSessionGeneration = next;
  saveUser(user);
  return next;
}
...
// Remove every login-bearing user linked to one kid profile. A child may have
// multiple provisioned credentials on the same logical kid account; deleting
// that account invalidates all of those passkeys/cookie sessions on the next
// request because currentUser() can no longer resolve the uid. The caller is
// responsible for clearing push registries keyed by the returned user ids.
function removeKidUsersForProfile(familyId, kidId) {
  const root = db.load();
  const removed = [];
  for (const [userId, user] of Object.entries(root.users || {})) {
    const link = user && user.data && user.data.kid;
    if (!link || link.familyId !== familyId || link.kidId !== kidId) continue;
    removed.push(userId);
...
//     createdAt, lastUsed, legacyIOSUVBypass? }
