...
module.exports = (app, deps) => {
  const { chat, notifications, store, family, trips, gifs, requireAuth, requireParent, requireFamily, userRole, gifLimiter, buzzLimiter: configuredBuzzLimiter } = deps;
  ...
  app.post("/api/chat/attachments", requireAuth, receiveAttachment, (req, res) => {
  ...
  app.get("/api/chat/attachments/:id", requireAuth, (req, res) => {
    ...
    const safeInline = /^(?:image\/(?:jpeg|png|gif|webp|heic|heif)|video\/(?:mp4|quicktime|mpeg))$/.test(meta.mimeType);
    const disposition = safeInline ? "inline" : "attachment";
    const encodedName = encodeURIComponent(meta.filename).replace(/'/g, "%27");
    res.set("Cache-Control", "private, max-age=3600");
    res.set("Content-Type", safeInline ? meta.mimeType : "application/octet-stream");
  ...
  app.post("/api/chat/buzz", requireAuth, buzzLimiter, requireFamily, async (req, res) => {
    const { text } = req.body || {};
    const isKid = userRole(req.user) === "kid";
    const result = chat.sendMessage(req.family.id, {
      senderType: isKid ? "kid" : "parent",
