...
const configuredAttachmentReads = Number(process.env.RL_CHAT_ATTACHMENT_READ_MAX);
const MAX_CONCURRENT_ATTACHMENT_READS = Number.isSafeInteger(configuredAttachmentReads) && configuredAttachmentReads > 0
  ? configuredAttachmentReads
  : 8;
...
function limitConcurrentAttachmentReads(req, res, next) {
  if (activeAttachmentReads >= MAX_CONCURRENT_ATTACHMENT_READS) {
    res.set("Retry-After", "1");
    return res.status(503).json({ error: "Media is busy — please try again in a moment." });
  }
  activeAttachmentReads++;
  let released = false;
  const release = () => {
    if (released) return;
    released = true;
...
module.exports = (app, deps) => {
  ...
  function currentDeliveryPayload(payload) {
    ...
    return Object.assign({}, payload, {
      familyParentIds: (currentFamily.parentIds || []).filter((id) => (!Array.isArray(payload.familyParentIds) || payload.familyParentIds.includes(id))),
      familyKidUserIds: store.listKidUserIdsForFamily(currentFamily.id).filter((id) =>
        (!Array.isArray(payload.familyKidUserIds) || payload.familyKidUserIds.includes(id))
        && family.familyForKidUser(store.getUser(id))?.id === currentFamily.id),
    });
  }
  ...
  function deliverChatNotification(kind, payload) {
    ...
    const message = chat.getMessage(payload.familyId, payload.messageId);
    if (!message || message.deleted) return Promise.resolve();
    Object.assign(currentPayload, { text: message.text, card: message.card, media: message.media });
  ...
  app.post("/api/chat/attachments", requireAuth, attachmentLimiter, limitConcurrentAttachmentUploads, receiveAttachment, async (req, res) => {
    ...
    try {
      const meta = await chatAttachments.saveAsync({
  ...
  app.get("/api/chat/attachments/:id", requireAuth, limitConcurrentAttachmentReads, async (req, res) => {
    let meta;
    ...
    let record;
    try { record = await chatAttachments.readAsync(meta.id, meta); } catch (e) {
      console.error("[chat] attachment read failed:", e.message);
      return res.status(500).json({ error: "Couldn't read that attachment." });
    }
    if (!record || !canReadAttachment(req, meta)) return res.status(404).json({ error: "Attachment not found." });
  ...
  app.get("/api/chat/messages", requireAuth, requireFamily, (req, res) => {
    ...
    function onMessage() {
      ...
      settled = true;
      cleanup();
      if (req.user && (!store.getUser(req.user.id) || familyForRequester(req)?.id !== familyId
          || (typeof deps.currentUser === "function" && !req.watchAuth && !deps.currentUser(req)))) {
  ...
  app.post("/api/chat/messages", requireAuth, requireFamily, (req, res) => {
    const { text, card, media, clientMessageId, expectedContext } = req.body || {};
    // Optional check-in guard: cookie/account changes cannot send an old draft
    // under another identity or family. This never selects the actor or scope.
    if (expectedContext !== undefined && (!expectedContext
        || expectedContext.userId !== req.user.id || expectedContext.familyId !== req.family.id
        || (expectedContext.role !== undefined && expectedContext.role !== userRole(req.user)))) {
      return res.status(409).json({ error: "Account changed. Cancel this draft and start again." });
    }
    ...
    if (!result.existing) queueChatNotification("chat_message", payload, result.message.id);
  ...
  app.get("/api/chat/rooms", requireAuth, (req, res) => {
    ...
    if (fam) rooms.push({ roomId: "family", title: fam.name || "Family chat", kind: "family" });
    // Everyone with a family login has a private thread with Hermes
    // (lib/hermes-threads.js; docs/HERMES-THREADS-CONTRACT.md).
    if (fam && hermesThreads.memberFor(fam, req.user.id)) rooms.push({ roomId: "hermes", title: "Hermes", kind: "assistant" });
    ...
    for (const trip of trips.allTrips()) {
      if (trips.accessFor(req.user, trip) === "member") {
        rooms.push({ roomId: "trip:" + trip.id, tripId: trip.id, title: trip.name, memberCount: trip.members.length, kind: "trip" });
      }
    }
