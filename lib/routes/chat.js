...
module.exports = (app, deps) => {
  ...
  notificationOutbox.configure({
    chat_message: (payload) => notifications.notifyChatMessage(payload),
    chat_buzz: (payload) => notifications.notifyChatBuzz(payload),
  });
  ...
  function queueChatNotification(kind, payload, messageId) {
    try {
      ...
      const direct = kind === "chat_buzz" ? notifications.notifyChatBuzz : notifications.notifyChatMessage;
      Promise.resolve().then(() => direct(payload)).catch((err) => console.error("[chat] fallback push failed:", err.message));
    }
  }
