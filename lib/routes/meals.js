...
module.exports = (app, deps) => {
  ...
  function chatSourceFromBody(req, body) {
    ...
    const message = chat.getMessage(req.family.id, body.sourceId);
    if (!message || message.deleted) return { error: "That chat message is not available." };
    ...
    if (message.roomId && message.roomId !== "family") return { error: "Trip chat messages cannot become shopping items." };
...
[{"date":"YYYY-MM-DD","slot":"dinner","title":"Dal Tadka","note":"short method in 1-2 sentences","ingredients":["toor dal","cumin seeds"],"prep":[{"label":"Soak toor dal","leadHours":2}],"proteinGPerPortion":18}]`;
  ...
  app.post("/api/meals/shopping", requireAuth, requireFamily, (req, res) => {
    ...
    const source = chatSourceFromBody(req, body);
