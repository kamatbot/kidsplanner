...
module.exports = (app, deps) => {
  ...
  function familyMemberUserIds(fam) {
    const ids = [...(fam.parentIds || [])];
    if (store && typeof store.listKidUserIdsForFamily === "function") {
      ids.push(...store.listKidUserIdsForFamily(fam.id));
    }
    return [...new Set(ids)];
  }

  function actionWatchRecipients(fam, action) {
    const all = familyMemberUserIds(fam);
    if (!action || action.assigneeType === "family") return all;
    if (action.assigneeType === "parent") return all.filter((id) => id === action.assigneeId || (fam.parentIds || []).includes(id));
    return all.filter((id) => {
      const user = store && typeof store.getUser === "function" ? store.getUser(id) : null;
      return user && user.data && user.data.kid && user.data.kid.kidId === action.assigneeId;
    }).concat(fam.parentIds || []);
  }

  function notifyWatchAction(req, action) {
    if (!notifications || typeof notifications.notifyWatchAction !== "function") return;
    Promise.resolve(notifications.notifyWatchAction({
      recipientUserIds: actionWatchRecipients(req.family, action),
  ...
  app.post("/api/family/actions", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    notifyWatchAction(req, result.action);
  ...
  app.patch("/api/family/actions/:id", requireAuth, requireFamily, (req, res) => {
    ...
    const body = req.body || {};
    const role = userRole(req.user);
    // A parent's reminder is a view of student work, not authority to
    // complete, reopen or hide that work through the action endpoint.
    if (role !== "kid" && existing.sourceType === "homework" &&
        (hasOwn(body, "status") || hasOwn(body, "snoozedUntil"))) {
      return res.status(403).json({ error: "Homework progress can only be changed by the student. Open the homework to review it." });
    }
    if (req.watchAuth && Object.keys(body).some((field) => !["status", "snoozedUntil"].includes(field))) {
      return res.status(403).json({ error: "Watch credentials can only complete or snooze actions." });
    }
    let patch;
    if (role === "kid") {
