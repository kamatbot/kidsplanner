...
module.exports = (app, deps) => {
  ...
  function postTripUpdateChatSafe(trip, actorUserId, text) {
    try {
  ...
  async function notifyTripEventSafe(trip, actorUserId, text) {
    postTripUpdateChatSafe(trip, actorUserId, text);
    try {
  ...
  app.post("/api/trips/:tripId/checklists", requireAuth, requireTripAnyAccess, (req, res) => {
    ...
    if (result.error) return res.status(400).json({ error: result.error });
    postTripUpdateChatSafe(req.trip, req.user.id, `started a packing list: ${result.checklist.title}`);
