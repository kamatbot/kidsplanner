...
module.exports = (app, deps) => {
  ...
  app.post("/api/trips/:tripId/chat/messages", requireAuth, requireTrip, requireTripMember, async (req, res) => {
    ...
    if (result.error) return res.status(400).json({ error: result.error });
    try {
      await notifications.notifyTripChatMessage(
        req.trip,
        req.user.id,
        resolveName(req.user.id) || "Trip guest",
        tripChatNotificationText(result.message),
        result.message.id,
      );
  ...
  app.post("/api/trips/:tripId/chat/buzz", requireAuth, buzzLimiter, requireTrip, requireTripMember, async (req, res) => {
    ...
    if (result.error) return res.status(400).json({ error: result.error });
    try {
      await notifications.notifyTripChatBuzz(req.trip, req.user.id, resolveName(req.user.id) || "Trip guest", result.message.text, result.message.id);
    } catch (e) { /* push must never block message send */ }
    res.json({ message: decorateTripMessage(req.trip.id, result.message) });
  });
