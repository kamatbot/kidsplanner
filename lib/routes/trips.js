...
module.exports = (app, deps) => {
  ...
  function deliverTripNotification(kind, payload) {
    ...
    if (payload.messageId) {
      message = chat.getMessage("trip:" + live.id, payload.messageId);
      if (!message || message.deleted) return;
    }
    const method = kind === "trip_chat_buzz" ? "notifyTripChatBuzz"
      : kind === "trip_chat_message" ? "notifyTripChatMessage" : "notifyTripEvent";
    return notifications[method](trip, payload.senderUserId, payload.senderName, payload.text, payload.messageId, message);
  }
  ...
  app.post("/api/trips/:tripId/chat/messages", requireAuth, requireTrip, requireTripMember, (req, res) => {
    ...
    if (!result.existing) queueTripNotification("trip_chat_message", req.trip, req.user.id, tripChatNotificationText(result.message), result.message.id);
    res.json({ message: decorateTripMessage(req.trip.id, result.message) });
  });

  app.post("/api/trips/:tripId/chat/buzz", requireAuth, buzzLimiter, requireTrip, requireTripMember, (req, res) => {
    const { text } = req.body || {};
    const result = chat.sendMessage("trip:" + req.trip.id, {
      senderType: "member",
      senderId: req.user.id,
