...
module.exports = (app, deps) => {
  ...
  app.get("/api/enrichment/puzzle/today", requireAuth, requireFamily, async (req, res) => {
    ...
    if (result.error) return res.status(400).json({ error: result.error });
    if (result.type === "crossword" && result.available) {
      // Session identity only: query parameters cannot select another child.
      const playerId = enrichmentPlayerId(req);
      if (!playerId) return res.status(403).json({ error: "No player profile linked to this session." });
      result.practiceWords = require("../crossword-practice").practiceWords(result.date, wordbank.listWords(playerId).words);
    }
    res.json(result);
  });
