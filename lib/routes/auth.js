...
const legacyIOSUV = require("../legacy-ios-uv");
...
module.exports = (app, deps) => {
  ...
  const legacyIOSUVBypassAllowed = deps.legacyIOSUVBypassAllowed || legacyIOSUV.isAllowed;
  const authWarning = deps.authWarning || ((message) => console.warn(message));
  const signupInviteCode = String(process.env.SIGNUP_INVITE_CODE || "fitodds").trim().toLowerCase();

  // Every successful authentication stamps the user's current server-side
  // session generation into the signed cookie. Logout bumps that generation,
  // invalidating copied/replayed older cookies without changing the signing key.
  function establishSession(req, user) {
    req.session.uid = user.id;
    req.session.authGen = store.sessionGeneration(user);
  }
  ...
  async function verifyRegistrationWithLegacyIOSFallback(req, params) {
    try {
      return {
        ...
        usedLegacyIOSUVBypass: false,
      };
    } catch (error) {
      if (!legacyIOSUVBypassAllowed(req, error)) throw error;
      const verification = await verifyRegistrationResponse({
        ...params,
        requireUserPresence: true,
        requireUserVerification: false,
      });
      authWarning("[auth] temporary legacy iOS user-verification bypass used for signup");
      return { verification, usedLegacyIOSUVBypass: true };
    }
  }

  async function verifyAuthenticationWithLegacyIOSFallback(req, cred, params) {
    try {
      return await verifyAuthenticationResponse({ ...params, requireUserVerification: true });
    } catch (error) {
      if (!cred.legacyIOSUVBypass || !legacyIOSUVBypassAllowed(req, error)) throw error;
      const verification = await verifyAuthenticationResponse({ ...params, requireUserVerification: false });
      authWarning("[auth] temporary legacy iOS user-verification bypass used for sign-in");
      return verification;
    }
  }
  ...
  app.post("/api/webauthn/auth/verify", authLimiter, async (req, res) => {
    ...
    let verification;
    try {
      verification = await verifyAuthenticationWithLegacyIOSFallback(req, cred, {
        response: req.body,
        expectedChallenge,
        expectedOrigin: origins,
        expectedRPID: rpID,
  ...
  app.post("/api/webauthn/signup/options", signupLimiter, async (req, res) => {
    ...
    const inviteCode = String((req.body && req.body.inviteCode) || "").trim().toLowerCase();
    if (!inviteCode || inviteCode !== signupInviteCode) {
      req.session.waSignup = undefined;
      return res.status(403).json({ error: "That invite code isn't valid. Check it and try again." });
    }
    const { rpID, rpName } = rpForRequest(req);
    ...
    req.session.waSignup = { challenge: options.challenge, userId: newId, name: displayName, inviteValidated: true };
    res.json(options);
  });
  ...
  app.post("/api/webauthn/signup/verify", signupLimiter, async (req, res) => {
    ...
    req.session.waSignup = undefined;
    if (!pending || pending.inviteValidated !== true) {
      return res.status(400).json({ error: "Sign-up session expired — start again." });
    }
    const { rpID, origins } = rpForRequest(req);
    let verification;
    let usedLegacyIOSUVBypass = false;
    try {
      const result = await verifyRegistrationWithLegacyIOSFallback(req, {
        response: req.body,
      ...
      usedLegacyIOSUVBypass = result.usedLegacyIOSUVBypass;
    } catch (e) {
    ...
    store.addCredential(user.id, {
      ...
      createdAt: new Date().toISOString(),
      ...(usedLegacyIOSUVBypass ? { legacyIOSUVBypass: true } : {}),
    });
    establishSession(req, user);
    try {
      const client = (req.get("x-fametc-client") || "").toLowerCase();
      const isAndroid = client === "android" || /FamETC-?Android/i.test(req.get("user-agent") || "");
      const isIOS = client === "ios" || /FamETC-?iOS/i.test(req.get("user-agent") || "");
      const source = isAndroid ? "android" : (isIOS ? "ios" : "web");
      analytics.recordSignup(source);
