...
module.exports = (app, deps) => {
  const { schoolAccount, schoolApi, moodleClient, family, homework, actions, requireAuth, requireParent, requireFamily, authLimiter } = deps;
  ...
  app.get("/api/school/status", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    res.json({
      ...
      feedEncryptionAvailable: schoolApi ? schoolApi.encryptionAvailable() : false,
      feedConnections: schoolApi ? schoolApi.listStatus(req.family.id) : [],
    });
  });

  // The child-feed URLs are bearer credentials. Only parents may save,
  // replace, synchronize, or remove them; responses expose status only and
  // never echo either URL or the shared capability code.
  app.post("/api/school/feeds", requireAuth, requireParent, requireFamily, authLimiter, async (req, res) => {
    if (!schoolApi || !schoolApi.encryptionAvailable()) {
      return res.status(503).json({ error: "School feed connection is not available because secure storage is not configured." });
    }
    const { kidId, homeworkUrl, timetableUrl } = req.body || {};
    if (!kidId || !family.kidBelongsToFamily(req.family.id, kidId)) {
      return res.status(400).json({ error: "Kid not found in this family." });
    }
    const saved = schoolApi.saveConnection(req.family.id, req.user.id, kidId, { homeworkUrl, timetableUrl });
    if (saved.error) return res.status(400).json({ error: saved.error });
    const sync = await schoolApi.syncKid(req.family.id, kidId, { force: true });
    res.set("Cache-Control", "no-store");
    res.json({ ok: true, status: sync.status || saved.status, sync: { ok: !!sync.ok, error: sync.error || null } });
  });

  app.post("/api/school/feeds/sync", requireAuth, requireParent, requireFamily, async (req, res) => {
    if (!schoolApi) return res.status(503).json({ error: "School feed synchronization is unavailable." });
    const kidId = req.body && req.body.kidId;
    if (kidId && !family.kidBelongsToFamily(req.family.id, kidId)) {
      return res.status(400).json({ error: "Kid not found in this family." });
    }
    const result = kidId
      ? await schoolApi.syncKid(req.family.id, kidId, { force: true })
      : await schoolApi.syncFamily(req.family.id, { force: true });
    res.set("Cache-Control", "no-store");
    res.json(result);
  });

  app.post("/api/school/feeds/disconnect", requireAuth, requireParent, requireFamily, (req, res) => {
    if (!schoolApi) return res.status(503).json({ error: "School feed connection is unavailable." });
    const kidId = req.body && req.body.kidId;
    if (!kidId || !family.kidBelongsToFamily(req.family.id, kidId)) {
      return res.status(400).json({ error: "Kid not found in this family." });
    }
    res.set("Cache-Control", "no-store");
    res.json(schoolApi.disconnect(req.family.id, kidId));
  });
