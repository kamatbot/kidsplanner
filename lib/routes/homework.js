...
module.exports = (app, deps) => {
  ...
  app.post("/api/homework", requireAuth, requireFamily, (req, res) => {
    const role = userRole(req.user);
    const body = req.body || {};
    if (role !== "kid" && Array.isArray(body.checklist) && body.checklist.length > 0) {
      return res.status(403).json({ error: "Homework steps can only be created by the student." });
    }
    ...
    const result = homework.addHomework(req.family.id, {
      ...
      notes: body.notes,
      checklist: role === "kid" ? body.checklist : undefined,
  ...
  app.patch("/api/homework/:id", requireAuth, requireFamily, (req, res) => {
    ...
    const body = req.body || {};
    if (role !== "kid" && (body.status !== undefined || body.checklist !== undefined)) {
      return res.status(403).json({ error: "Homework planning and progress can only be changed by the student." });
    }
  ...
  app.patch("/api/homework/:id/checklist/:index", requireAuth, requireFamily, (req, res) => {
    ...
    if (!existing) return res.status(404).json({ error: "Homework item not found." });
    if (role !== "kid") {
      return res.status(403).json({ error: "Homework progress can only be changed by the student." });
    }
    if (!homework.canAccess(req.user, role, req.family.id, existing)) {
      return res.status(403).json({ error: "You don't have access to this homework item." });
    }
