...
function mount(app, deps) {
  ...
  app.get("/api/pathodds/today", requireAuth, requireFamily, async (req, res) => {
    ...
    if (resolved.error) return res.status(resolved.status).json({ error: resolved.error });
    const grant = grants.getGrant(resolved.subject.id);
    if (!hasActiveGrant(grant)) return res.json({ linked: false, linkUrl: linkUrl("sat.home"), childView: resolved.isChildView });
    let pairwise;
    try {
      pairwise = pairwiseFor(resolved);
    } catch (error) {
  ...
  app.post("/api/pathodds/launch", requireAuth, requireFamily, async (req, res) => {
    ...
    const grant = grants.getGrant(resolved.subject.id);
    if (!hasActiveGrant(grant)) return res.status(409).json({ error: "Connect PathOdds first.", linkUrl: linkUrl(route), code: "not-linked" });
    let pairwise;
    try {
      pairwise = pairwiseFor(resolved);
    } catch (error) {
...
module.exports = { mount, resolveSubject, linkUrl, validTimeZone, hasActiveGrant, WEBHOOK_PATH };
