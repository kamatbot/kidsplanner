...
module.exports = (app, deps) => {
    ...
    if (actor) {
      ...
      if (auth && room.kind === "family") {
        try {
          ...
          // Conversational replies may continue without widening authority.
        }
      }
    }
    return decorated;
  ...
  function operatorErrorStatus(error) {
    ...
    if (["APPROVAL_HASH_MISMATCH", "APPROVAL_NOT_PENDING", "APPROVAL_EXPIRED"].includes(code)) return 409;
    if (["OPERATOR_CASE_NOT_FOUND", "APPROVAL_NOT_FOUND", "OPERATOR_FAMILY_NOT_FOUND"].includes(code)) return 404;
    return 400;
  }
  ...
  // The bridge previously returned a flat snapshot consumed by existing
  // Hermes adapter/tests. Canonical Family Context v1 is authoritative. These
  // aliases are read-only projections derived from it. Exclusion vocabulary is
  // omitted on this compatibility endpoint so a sensitive field name never
  // appears merely to announce that the field itself was withheld.
  function withLegacyContextAliases(snapshot) {
    ...
    const members = sections.identities && Array.isArray(sections.identities.members)
      ? sections.identities.members : [];
    const kids = members.filter((member) => member.role === "kid").map((member) => ({
      id: member.kidId,
  ...
  app.get("/api/operator/cases", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    const fam = primaryFamily(req);
    try {
      return res.json({ cases: operatorCards.listCaseCards(fam.id, req.user.id, { state: req.query && req.query.state, limit: req.query && req.query.limit }) });
  ...
  app.get("/api/operator/cases/:caseId", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    const fam = primaryFamily(req);
    try {
      const card = operatorCards.caseCard(fam.id, req.params.caseId, req.user.id);
      if (!card) return res.status(404).json({ error: "Operator case not found.", code: "OPERATOR_CASE_NOT_FOUND" });
      return res.json({ case: card });
    } catch (error) { return sendOperatorError(res, error); }
  });
  ...
  app.get("/api/operator/approvals", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    const fam = primaryFamily(req);
    try {
      const approvals = operatorExecution.listApprovalsForParent(fam.id, req.user.id, { state: req.query && req.query.state, limit: req.query && req.query.limit });
      return res.json({ approvals, supportedActionTypes: operatorExecution.supportedActionTypes() });
    } catch (error) { return sendOperatorError(res, error); }
  });
  ...
  app.get("/api/hermes/rooms/:roomId/context", requireHermes, (req, res) => {
    ...
    try {
      ...
      const snapshot = familyContext.buildFamilyContext(req.hermesAuth.family.id, {
        actor: null,
  ...
  app.get("/api/hermes/rooms/:roomId/messages", requireHermes, (req, res) => {
    ...
    const read = () => {
      ...
      return { messages: result.messages.map((message) => decorateMessage(room, message, req.hermesAuth)), cursor: result.cursor };
    };
