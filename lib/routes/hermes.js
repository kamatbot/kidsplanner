...
const operatorLiveExecution = require("../operator-live-execution");
...
module.exports = (app, deps) => {
  ...
  // What Hermes (including the Mac's reminder loop) last said in a private
  // thread, so a reply is read against the nudge it answers.
  function recentHermes(room, message) {
    return chat.listMessages(room.scopeKey, { limit: 30 })
      .filter((m) => m.senderType === "agent" && !m.deleted && m.text && m.createdAt <= message.createdAt)
      .slice(-5)
      .map((m) => ({ at: m.createdAt, text: String(m.text).slice(0, 400), kind: m.card && m.card.type === "hermes-nudge" ? m.card.kind : null }));
  }
  function decorateMessage(room, message, auth) {
    ...
    if (room.kind === "assistant") decorated.recentHermes = recentHermes(room, message);
    const actor = operator.actorFromMessage(message, senderName);
    if (actor) {
      decorated.actor = actor;
      if (auth && (room.kind === "family" || room.kind === "assistant")) {
  ...
  function threadOwnerActor(member) {
    return member.role === "kid"
      ? { type: "kid", principalId: member.kidId, kidId: member.kidId, userId: member.userId, name: member.name }
      : { type: "parent", principalId: member.userId, userId: member.userId, kidId: null, name: member.name };
  }
  // Kid sessions see only their own homework/actions and their own or
  // family-wide calendar items (APP-BRIEF kid scope); siblings' rows go.
  function scopeSnapshotToKid(snapshot, kidId) {
    const sections = snapshot && snapshot.sections;
    if (!sections) return;
    for (const name of ["calendar", "homework", "actions"]) {
      const section = sections[name];
      const key = Array.isArray(section && section.items) ? "items" : Array.isArray(section && section.events) ? "events" : null;
      if (key) section[key] = section[key].filter((item) => !item || !item.kidId || item.kidId === kidId);
    }
  }
  ...
  app.post("/api/operator/approvals/:approvalId/decision", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    try {
      ...
      // Keep the matching Hermes-thread approval cards in step with this decision.
      try {
        const who = (req.user.data && req.user.data.profile && req.user.data.profile.name || "").split(/\s+/)[0] || "a parent";
        hermesThreads.resolveApprovalCards(fam, req.params.approvalId, `${result.approval.state === "approved" ? "Approved" : "Rejected"} by ${who}`, { by: req.user.id });
      } catch (_) { /* the decision stands either way */ }
      if (result.approval.state === "approved") {
        result.continuation = operatorLiveExecution.continueApproved(fam.id, req.params.approvalId, req.user.id);
        result.execution = operatorExecution.getExecutionForApproval(fam.id, req.params.approvalId);
      }
      return res.json(result);
    }
    catch (error) { return sendOperatorError(res, error); }
  ...
  // Transport wake-up only: authority/payload come from persisted exact approvals.
  app.post("/api/hermes/continuations", requireHermes, (req, res) => {
    setNoStore(res);
    if (Object.keys(req.body || {}).length) return res.status(400).json({ error: "Continuation requests take no actor or action input." });
    try { return res.json({ continuations: operatorLiveExecution.drainContinuations(req.hermesAuth.family.id) }); }
    catch (error) { return sendOperatorError(res, error); }
  });
  ...
  app.get("/api/hermes/rooms/:roomId/context", requireHermes, (req, res) => {
    ...
    if (room.kind !== "family" && room.kind !== "assistant") return res.status(403).json({ error: FAMILY_CONTEXT_ROOM_ERROR });
    try {
      const query = req.query || {};
      // A private thread is read with its owner's authority: a kid's Hermes
      // sees the kid ceiling and only that kid's own items.
      const actor = room.kind === "assistant" ? threadOwnerActor(room.member) : null;
      const snapshot = familyContext.buildFamilyContext(req.hermesAuth.family.id, { actor, purpose: "family-assistance", roomId: room.roomId, from: query.from, to: query.to });
      if (actor && actor.type === "kid") scopeSnapshotToKid(snapshot, actor.kidId);
  ...
  app.post("/api/hermes/rooms/:roomId/messages", requireHermes, async (req, res) => {
    ...
    try {
      // A private thread notifies its owner only.
      if (room.kind === "assistant") await notifications.notifyHermesThread({ userId: room.member.userId, familyId: req.hermesAuth.family.id, text: result.message.text, messageId: result.message.id });
      else if (room.kind === "family") await notifications.notifyChatMessage({ familyParentIds: req.hermesAuth.family.parentIds, familyKidUserIds: store.listKidUserIdsForFamily(req.hermesAuth.family.id), senderUserId: null, senderName: "Hermes", familyId: req.hermesAuth.family.id, text: result.message.text });
