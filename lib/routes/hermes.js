...
module.exports = (app, deps) => {
  const { hermes, family, chat, store, notifications, requireAuth, requireParent, requireFamily, requireAdmin } = deps;
  ...
  // Admin-only beta operations. requireAdmin is token-guarded by server.js and
  // is intentionally separate from ordinary parent sessions.
  if (typeof requireAdmin === "function") {
    app.get("/api/admin/operator-beta/dashboard", requireAdmin, (req, res) => {
      setNoStore(res);
      try { return res.json(operatorBeta.dashboard()); }
      catch (error) { return sendOperatorError(res, error); }
    });
    app.post("/api/admin/operator-beta/global", requireAdmin, (req, res) => {
      setNoStore(res);
      try { return res.json({ global: operatorBeta.setGlobal(req.body || {}) }); }
      catch (error) { return sendOperatorError(res, error); }
    });
    app.get("/api/admin/operator-beta/families/:familyId", requireAdmin, (req, res) => {
      setNoStore(res);
      try { return res.json(operatorBeta.familyDashboard(req.params.familyId)); }
      catch (error) { return sendOperatorError(res, error); }
    });
    app.post("/api/admin/operator-beta/families/:familyId", requireAdmin, (req, res) => {
      setNoStore(res);
      try { return res.json({ config: operatorBeta.setFamilyConfig(req.params.familyId, req.body || {}) }); }
      catch (error) { return sendOperatorError(res, error); }
    });
    app.post("/api/admin/operator-beta/prune-evidence", requireAdmin, (req, res) => {
      setNoStore(res);
