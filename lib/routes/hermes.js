...
module.exports = (app, deps) => {
  ...
  function decorateMessage(room, message, auth) {
    ...
    if (actor) {
      ...
      // Trip rooms are shared with external members. They never receive
      // Family Operator authority, even when the sender is a family parent.
  ...
  // static metadata only; family status and credentials remain parent-only.
  ...
  app.get("/api/operator/cases", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    try {
      return res.json({
        cases: operatorCards.listCaseCards(fam.id, req.user.id, {
  ...
  app.get("/api/operator/activity", requireAuth, requireParent, requireFamily, (req, res) => {
    ...
    const fam = primaryFamily(req);
    try {
      return res.json({ activity: operatorCards.familyActivity(fam.id, req.user.id, { limit: req.query && req.query.limit }) });
    } catch (error) {
      return sendOperatorError(res, error);
    }
  });
  ...
  app.get("/api/hermes/rooms/:roomId/context", requireHermes, (req, res) => {
    ...
    try {
      ...
      return res.json(familyContext.buildFamilyContext(req.hermesAuth.family.id, {
        actor: null,
