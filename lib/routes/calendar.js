...
function chatSourceFromBody(body) {
  ...
  if (body.sourceType !== "chat") return { error: "Only family chat messages can be used as event sources." };
...
module.exports = (app, deps) => {
  ...
  app.post("/api/calendar/events", requireAuth, requireFamily, (req, res) => {
    ...
    if (source.sourceId && chat && typeof chat.getMessage === "function") {
      const sourceMessage = chat.getMessage(req.family.id, source.sourceId);
      if (!sourceMessage || sourceMessage.deleted || !sourceMessage.text) {
        return res.status(400).json({ error: "The family chat message is unavailable." });
      }
    }
    ...
    res.json({ event: result.event, existing: !!result.existing });
  });
