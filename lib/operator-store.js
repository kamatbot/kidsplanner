 ...
 * AES-256-GCM key used by the rest of FamETC whenever DATA_ENCRYPTION_KEY is
 * configured. Structural fields needed for indexes (family/state/timestamps)
 * remain plaintext.
 */
...
function encodeSecret(value) {
  ...
  return key ? datacrypto.encrypt(raw, key) : raw;
}
...
function decodeSecret(value, { json = false } = {}) {
  ...
  if (datacrypto.isEncrypted(raw)) {
    const key = datacrypto.loadKey();
    if (!key) throw new OperatorStorageUnavailableError(new Error("Operator data is encrypted but DATA_ENCRYPTION_KEY is not set."));
    raw = datacrypto.decrypt(raw, key);
  }
  if (!json) return raw;
  try {
