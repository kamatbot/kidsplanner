 ...
 *     "school-portal"|"school-api"|"manual"|"ai", sourceUid?, moodleIdentity?, notes?,
 *     checklist: [{text, done}],
 *     createdAt, updatedAt }
 *
...
const SOURCES = new Set(["school", "school-portal", "school-api", "manual", "ai"]);
...
function updateHomework(familyId, id, patch = {}) {
  ...
  if (previousStatus !== "done" && item.status === "done") require("./fams").awardHomework(familyId, item);
  db.persist();
...
// Reconcile one child's read-only St Andrews API snapshot. Remote task ids are
// stable, so they are the source identity; student-owned status, notes and
// checklist state survive descriptive feed updates. A matching legacy Moodle
// row is adopted once to avoid duplicating homework during the migration away
// from the Chrome extension. Tasks absent from a successful snapshot are no
// longer outstanding at school and are removed from FamETC.
function syncSchoolApi(familyId, kidId, remoteItems = []) {
  const fam = family.getFamily(familyId);
  if (!fam || !fam.kids.some((kid) => kid.id === kidId)) {
    return { error: "Kid not found in this family.", created: 0, updated: 0, removed: 0, removedIds: [] };
  }
  if (!Array.isArray(remoteItems)) {
    return { error: "Invalid school homework snapshot.", created: 0, updated: 0, removed: 0, removedIds: [] };
  }

  const list = famList(familyId);
  const liveSourceUids = new Set();
  let created = 0;
  let updated = 0;

  for (const raw of remoteItems) {
    if (!raw || !/^\d+$/.test(String(raw.sourceTaskId || "")) || !String(raw.title || "").trim()) continue;
    const sourceTaskId = String(raw.sourceTaskId);
    const sourceUid = `sta-api:${kidId}:${sourceTaskId}`;
    liveSourceUids.add(sourceUid);

    let item = list.find((candidate) => candidate.kidId === kidId
      && candidate.source === "school-api" && candidate.sourceUid === sourceUid);
    if (!item) {
      item = list.find((candidate) => candidate.kidId === kidId
        && candidate.source === "school-portal"
        && candidate.moodleIdentity
        && String(candidate.moodleIdentity.taskId) === sourceTaskId);
      if (item) {
        moodleCompletionOutbox.cancelPendingInMemory(familyId, item.id);
        item.source = "school-api";
        item.sourceUid = sourceUid;
        delete item.moodleIdentity;
      }
    }
    ...
    const sourceFields = {
      ...
      dueDate: raw.dueDate && sanitizeDueDate(raw.dueDate) ? raw.dueDate : null,
      effortMin: Number.isFinite(Number(raw.effortMin)) && Number(raw.effortMin) > 0
        ? Math.round(Number(raw.effortMin)) : null,
      sourceTaskId,
      sourceDueDate: sanitizeDueDate(raw.sourceDueDate),
      noDueDate: raw.noDueDate === true,
      schoolDescription: String(raw.description || "").trim().slice(0, 2000),
      schoolLink: String(raw.link || "").trim().slice(0, 2000),
      schoolCode: String(raw.schoolCode || "").trim().slice(0, 80),
      schoolSetBy: String(raw.setBy || "").trim().slice(0, 120),
      selfSet: raw.selfSet === true,
    };

    if (item) {
      const notesWereSourceOwned = !item.notes || item.notes === item.schoolDescription;
      Object.assign(item, sourceFields, { source: "school-api", sourceUid, updatedAt: new Date().toISOString() });
      if (notesWereSourceOwned) item.notes = sourceFields.schoolDescription;
      updated += 1;
    } else {
      ...
      item = Object.assign({
        ...
        source: "school-api",
        sourceUid,
        notes: sourceFields.schoolDescription,
        checklist: [],
        createdAt: now,
        updatedAt: now,
      }, sourceFields);
  ...
  const removedIds = [];
  root().homework[familyId] = list.filter((item) => {
    const stale = item.kidId === kidId && item.source === "school-api" && !liveSourceUids.has(item.sourceUid);
    if (stale) {
  ...
  if (created || updated || removedIds.length) db.persist();
  return { created, updated, removed: removedIds.length, removedIds };
}

// One-time migration used when a child first connects the official feeds.
// Validate the complete normalized snapshot before removing anything, then
// replace every homework source for that child with the feed's outstanding
// tasks. Other children and non-homework family data are untouched.
function replaceKidHomeworkWithSchoolApi(familyId, kidId, remoteItems = []) {
  const fam = family.getFamily(familyId);
  if (!fam || !fam.kids.some((kid) => kid.id === kidId)) {
    return { error: "Kid not found in this family.", created: 0, updated: 0, removed: 0, removedIds: [] };
  }
  if (!Array.isArray(remoteItems) || remoteItems.some((raw) => (
    !raw || !/^\d+$/.test(String(raw.sourceTaskId || "")) || !String(raw.title || "").trim()
  ))) {
    return { error: "Invalid school homework snapshot.", created: 0, updated: 0, removed: 0, removedIds: [] };
  }

  const list = famList(familyId);
  const removedIds = list.filter((item) => item.kidId === kidId).map((item) => item.id);
  for (const id of removedIds) moodleCompletionOutbox.cancelPendingInMemory(familyId, id);
  root().homework[familyId] = list.filter((item) => item.kidId !== kidId);

  const synced = syncSchoolApi(familyId, kidId, remoteItems);
  if (synced.error) return synced;
  db.persist();
  return Object.assign({}, synced, {
    replaced: removedIds.length,
    removed: removedIds.length,
    removedIds,
  });
}

function removeSchoolApiForKid(familyId, kidId) {
  const list = famList(familyId);
  const removedIds = list
    .filter((item) => item.kidId === kidId && item.source === "school-api")
    .map((item) => item.id);
  if (!removedIds.length) return { removed: 0, removedIds: [] };
  const removedSet = new Set(removedIds);
  for (const id of removedIds) moodleCompletionOutbox.cancelPendingInMemory(familyId, id);
  root().homework[familyId] = list.filter((item) => !removedSet.has(item.id));
  db.persist();
...
module.exports = {
  ...
  removeBySource,
  syncSchoolApi,
  replaceKidHomeworkWithSchoolApi,
  removeSchoolApiForKid,
  toggleChecklistItem,
