...
const SERVER_INFO = Object.freeze({ name: "fametc-family-operator", version: "0.1.0" });
const SERVER_INSTRUCTIONS = "Use FamETC tools only for the authenticated family. Create a durable case for multi-step work. Never invent an actor: context reads and case creation require the actorToken attached to the initiating FamETC message. Request approval before irreversible external actions.";
...
const ACTOR_TOKEN = { type: "string", minLength: 20, maxLength: 5000, description: "Signed actor token attached by FamETC to the initiating @Hermes message." };
...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    description: "Read one durable Operator case owned by the authenticated family.",
    inputSchema: objectSchema({
      caseId: CASE_ID,
  ...
  },
  {
    ...
    description: "List recent durable Operator cases for the authenticated family.",
    inputSchema: objectSchema({
  ...
  },
  {
    ...
    description: "Move a case through the allowed state machine. Invalid jumps are rejected by FamETC policy.",
    inputSchema: objectSchema({
      caseId: CASE_ID,
      state: { type: "string", enum: operatorStore.CASE_STATES },
  ...
  },
  {
    ...
    description: "Append a typed, auditable step to a durable case. Use an idempotencyKey for external work that may be retried.",
    inputSchema: objectSchema({
      caseId: CASE_ID,
  ...
  },
  {
    ...
    description: "Persist an exact proposed action for parent approval. This does not execute the action. The returned actionHash identifies the payload that was approved/rejected later.",
    inputSchema: objectSchema({
      caseId: CASE_ID,
...
function hermesActor() {
  return { type: "agent", principalId: "hermes", userId: "hermes", name: "Hermes" };
}
...
function toolData(auth, name, args = {}) {
  ...
  switch (name) {
    ...
    case "fametc_cases_get": {
      ...
      const found = operator.getCase(familyId, caseId, { includeChildren: args.includeChildren === true });
      if (!found) throw new McpRequestError("Case not found.", -32602);
      return found;
    }
    case "fametc_cases_list":
      return { cases: operator.listCases(familyId, { state: args.state, limit: args.limit }) };
    ...
    case "fametc_approvals_request": {
      ...
      const approval = operator.requestApproval(familyId, requireString(args, "caseId"), {
        actor: hermesActor(),
        approverUserId: args.approverUserId,
        actionType: requireString(args, "actionType"),
        action: args.action,
