"use strict";
/** Canonical server word bank. Web and native daily activities both request
 * /api/enrichment/vocabulary/today; the native placement sampler is separate.
 * Keep the legacy thirty entries stable for historical daily/weekly content.
 */
...
const LEGACY_DAILY_WORDS = WORDS.slice();
const NEW_DAILY_WORDS = LESSONS.map(([word, pos, def, example]) => ({ word, pos, def, example }));
WORDS.push(...NEW_DAILY_WORDS);
// New words come first to leave more than six months between any old daily
// word and its first appearance after cutover. Never reorder this calendar.
const DAILY_WORDS = [...NEW_DAILY_WORDS, ...LEGACY_DAILY_WORDS];
// The weekly curriculum shares the same persistent word bank and quiz API.
const { ROOT_WORDS } = require("./weekly-learning");
const knownWords = new Set(WORDS.map(({ word }) => word.toLowerCase()));
for (const word of ROOT_WORDS.flatMap(root => root.words)) {
  if (!knownWords.has(word.word.toLowerCase())) {
    WORDS.push({ word: word.word, pos: word.pos, def: word.def, example: word.example });
    knownWords.add(word.word.toLowerCase());
  }
}
module.exports = { WORDS, DAILY_WORDS, LEGACY_DAILY_WORDS };
