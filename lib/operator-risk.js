...
const POLICIES = Object.freeze({
  ...
  "calendar.update": Object.freeze({ riskLevel: "low", allowedActors: ["parent", "kid"], approvalPolicy: "single-parent", dualParent: false, executable: true }),
  "action.create": Object.freeze({ riskLevel: "low", allowedActors: ["parent", "kid"], approvalPolicy: "single-parent", dualParent: false, executable: true }),
  "action.update": Object.freeze({ riskLevel: "low", allowedActors: ["parent", "kid"], approvalPolicy: "single-parent", dualParent: false, executable: true }),
  "trip.itinerary.update": Object.freeze({ riskLevel: "low", allowedActors: ["parent"], approvalPolicy: "single-parent", dualParent: false, executable: true }),
...
class OperatorRiskError extends Error {
  constructor(message, code = "OPERATOR_ACTION_POLICY_DENIED") { super(message); this.name = "OperatorRiskError"; this.code = code; }
}
...
function evaluate(actionType, actorType) {
  ...
  if (!policy) return { actionType: String(actionType || "").trim(), riskLevel: "critical", allowed: false, executable: false, approvalPolicy: "prohibited", dualParent: false, reason: "unregistered-action-type" };
  ...
  if (policy.approvalPolicy === "dual-parent" && policy.executable) return { ...policy, allowed: false, reason: "dual-parent-execution-not-implemented" };
...
function requireExecutable(actionType) {
  const policy = getPolicy(actionType);
  if (!policy || policy.approvalPolicy === "prohibited") throw new OperatorRiskError(`Action ${String(actionType || "(missing)")} is not executable.`, "OPERATOR_ACTION_PROHIBITED");
  if (policy.dualParent) throw new OperatorRiskError("This action requires dual-parent approval, which is not executable yet.", "OPERATOR_DUAL_PARENT_REQUIRED");
  if (!policy.executable) throw new OperatorRiskError(`No approved execution driver is enabled for ${actionType}.`, "EXECUTION_UNSUPPORTED_ACTION");
  return { actionType, ...policy };
}
function registry() { return Object.entries(POLICIES).map(([actionType, policy]) => ({ actionType, ...policy })); }
module.exports = { POLICIES, OperatorRiskError, getPolicy, evaluate, requireProposalAllowed, requireExecutable, registry };
