...
const SOURCE_TYPE_RE = /^[a-z][a-z0-9_-]{0,31}$/;
const SOURCE_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/;
...
function cleanSource(sourceType, sourceId) {
  ...
  if (!hasType || !hasId) return { error: "Source type and source id must be provided together." };

  const type = String(sourceType).trim();
  const id = String(sourceId).trim();
  if (!SOURCE_TYPE_RE.test(type)) return { error: "Invalid event source type." };
  if (!SOURCE_ID_RE.test(id)) return { error: "Invalid event source id." };
...
function addEvent(familyId, { title, date, time, endTime, notes, category, kidId, endDate, repeat, repeatUntil, createdBy, sourceType, sourceId } = {}) {
  const fam = family.getFamily(familyId);
  if (!fam) return { error: "Family not found." };
  const source = cleanSource(sourceType, sourceId);
  ...
  if (source.sourceType && source.sourceId) {
    event.sourceType = source.sourceType;
...
function getBySource(familyId, sourceType, sourceId) {
  return famList(familyId).find((e) => e.sourceType === sourceType && e.sourceId === sourceId) || null;
}
...
function legacyExtensionImportKind(event, kidId) {
  if (!event || event.kidId !== kidId) return null;
  if (event.sourceType === "eca") return "activities";
  if (event.sourceType) return null;
  if (event.source === "timetable-import") return "timetable";
  if (event.category !== "school") return null;

  const notes = String(event.notes || "");
  if (notes === "Timetable") return "timetable";
  if (notes === "Signed up activity") return "activities";
  if (!notes.startsWith("Import warning — needs review:")) return null;
  if (notes.includes("Raw values — day:") && notes.includes("; period:")) return "timetable";
  if (notes.includes("; clubId:") || notes.includes("; timeslot:")) return "activities";
  if (event.title === "Imported timetable item — needs review") return "timetable";
  if (event.title === "Imported activity — needs review") return "activities";
  return null;
}

function removeLegacyExtensionSchoolEvents(familyId, kidId) {
  const list = famList(familyId);
  const removed = { timetable: 0, activities: 0, removedIds: [] };
  for (let index = list.length - 1; index >= 0; index -= 1) {
    const kind = legacyExtensionImportKind(list[index], kidId);
    if (!kind) continue;
    removed[kind] += 1;
    removed.removedIds.push(list[index].id);
    list.splice(index, 1);
  }
  if (removed.removedIds.length) db.persist();
  return removed;
}
...
module.exports = {
  ...
  getBySource,
  removeEvent,
  removeLegacyExtensionSchoolEvents,
  updateEvent,
  canManage,
  CATEGORIES,
};
