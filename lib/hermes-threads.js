...
function dinnerIdeas(ctx) {
  ...
  if (!ideas.length) {
    ...
    // Late in the day only one idea may fit the time left.
    const some = ideas.length === 1 ? "is one" : `are ${ideas.length}`, s = ideas.length === 1 ? "" : "s";
    followUp(ctx, "ideas", busy
      ? `It's a ${busy.toLowerCase()} night, so here ${some} quick idea${s} for tonight:`
      : `Here ${some} idea${s} for tonight:`, {
      kind: "dinner-ideas",
