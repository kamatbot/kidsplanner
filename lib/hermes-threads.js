...
function suggestDinners(fam, { count, today, quick = false }) {
  const p = proactive();
  ...
  const ordered = quick ? [...pool.filter(isQuick), ...pool.filter((r) => !isQuick(r))] : pool;
  const out = [];
  const seen = new Set();
...
function dinnerIdeas(ctx) {
  ...
  const ideas = suggestDinners(ctx.fam, { count: 3, today, quick: !!busy });
  if (!ideas.length) {
