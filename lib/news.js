...
// Fixed, allow-listed first-party feeds keep this proxy useful without making
// it an arbitrary fetcher. Child-focused NASA feeds come first, with the broad
// NASA feed supplying fresh stories when those slower feeds have no new item.
const FEEDS = Object.freeze([
  { url: "https://www.nasa.gov/learning-resources/feed/", source: "NASA STEM", hosts: ["nasa.gov"] },
  { url: "https://science.nasa.gov/category/learning-resources/for-kids-and-students/feed/", source: "NASA Kids", hosts: ["nasa.gov"] },
  { url: "https://www.nasa.gov/feed/", source: "NASA", hosts: ["nasa.gov"] },
]);
...
function parseEntry(block, feed) {
  ...
  const cat = categoryFor(extractTag(block, ["category", "dc:subject"]), headline);
  return {
    id: url,
    cat,
    headline,
...
async function getRecentNews(options = {}) {
  ...
  try {
    ...
    // Existing focused callers that inject a fetch keep the original one-feed
    // behavior unless they explicitly provide feeds. Production uses all
    // allow-listed feeds.
    const feeds = options.feeds || ((options.fetch || options.fetchImpl) ? [FEEDS[2]] : FEEDS);
