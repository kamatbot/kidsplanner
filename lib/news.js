...
const FEEDS = Object.freeze([
  ...
  { url: "https://www.dogonews.com/category/science.rss", source: "DOGO News", hosts: ["dogonews.com"], production: true, defaultCategory: "🔬 Science" },
  { url: "https://www.dogonews.com/category/sports.rss", source: "DOGO News", hosts: ["dogonews.com"], production: true, defaultCategory: "Sports" },
  { url: "https://www.dogonews.com/category/fun.rss", source: "DOGO News", hosts: ["dogonews.com"], production: true, defaultCategory: "Culture" },
  { url: "https://www.dogonews.com/category/world.rss", source: "DOGO News", hosts: ["dogonews.com"], production: true, defaultCategory: "🌍 World" },
  { url: "https://www.dogonews.com/category/environment.rss", source: "DOGO News", hosts: ["dogonews.com"], production: true, defaultCategory: "🌿 Environment" },
  { url: "https://newsforkids.net/feed/", source: "NewsForKids.net", hosts: ["newsforkids.net"], production: true, defaultCategory: "🌍 World" },
  { url: "https://www.cbc.ca/kidsnews/ajax/news_feed", source: "CBC Kids News", hosts: ["cbc.ca"], production: true, defaultCategory: "Culture" },
  { url: "https://feeds.bbci.co.uk/newsround/rss.xml", source: "BBC Newsround", hosts: ["bbci.co.uk", "bbc.co.uk", "bbc.com"], production: true, defaultCategory: "🌍 World" },
  { url: "https://www.bangkokpost.com/rss/data/learning.xml", source: "Bangkok Post Learning", hosts: ["bangkokpost.com"], production: true, defaultCategory: "Local/Regional" },
]);
// These requested sources are not silently replaced with adult/general feeds.
// Activation requires a usable publisher feed and any necessary permission.
const SOURCE_NOTES = Object.freeze([
  { source: "Smithsonian TweenTribune", status: "Current feed not verified; Smithsonian Magazine is not TweenTribune." },
  { source: "CNN10", status: "Dedicated article feed not verified; do not substitute general CNN news." },
  { source: "The Week Junior", status: "Dedicated current article feed not verified." },
  { source: "CNA Explains", status: "RSS terms limit use to personal/non-commercial; permission needed before activation." },
  { source: "The Straits Times: Mind Stretcher/Youth", status: "Dedicated youth feed and reuse permission not verified." },
]);
...
const DAILY_CATEGORIES = Object.freeze([
  { category: "regional", label: "Local Innovation" },
  { category: "science", label: "Global Science & Discovery" },
  { category: "culture", label: "Culture, Sports & Human Interest" },
]);
...
// Screen the complete feed text before truncating the displayed excerpt.
// Positive innovation evidence is required for local news; an empty slot is
// preferable to a general-news replacement. This also protects crossword inputs.
const MATURE_NEWS = /\b(?:murder\w*|homicide\w*|kill(?:s|ed|er|ers|ing)?|sex(?:ual\w*|ting)?|porn\w*|rape\w*|rapist\w*|molest\w*|assault\w*|abuse\w*|prostitut\w*|traffick\w*|suicid\w*|self[-\s]harm\w*|shoot(?:s|ing|ings|er|ers)?|shot|stab\w*|bomb\w*|terroris\w*|weapon\w*|gun(?:s|man|men|fire)?|crime\w*|criminal\w*|gambl\w*|narcotic\w*|cocaine|heroin|methamphetamine|scam\w*|fraud\w*)\b/i;
const INNOVATION_NEWS = /\b(?:innovat\w*|invent\w*|prototype\w*|robot\w*|renewable\s+energy|solar\s+(?:panel\w*|cell\w*|power)|clean\s+(?:energy|technology)|electric\s+(?:vehicle\w*|bus\w*)|artificial\s+intelligence|scientific\s+(?:breakthrough\w*|discover\w*)|researchers?\s+(?:develop\w*|creat\w*|design\w*)|new\s+(?:technolog\w*|telescope\w*|satellite\w*|material\w*|battery|batteries))\b/i;

// In-memory parser proof survives truncation without exposing full feed content.
const FEED_SCREENED = Symbol("feed-screened");

function suitableNews(item, fullText = "") {
  const text = `${item.headline} ${item.summary} ${item.cat} ${item.url} ${fullText}`
    .normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "");
  if (MATURE_NEWS.test(text)) return false;
  const local = dailyCategory(item) === "regional" || /bangkokpost\.com|Bangkok Post/i.test(`${item.url} ${item.source}`);
  return !local || item[FEED_SCREENED] === true || INNOVATION_NEWS.test(`${item.headline} ${item.summary} ${fullText}`);
}
...
function parseEntry(block, feed) {
  ...
  if (feed.topicFilter && !feed.topicFilter.test(`${headline} ${summary}`)) return null;
  ...
  const fullText = cleanText(extractTags(block, ["title", "content:encoded", "description", "summary", "content", "category", "dc:subject"]).join(" "), MAX_BODY_BYTES);
  if (!suitableNews({ headline, summary, cat, url, source: feed.source }, fullText)) return null;
  return {
    id: url,
    cat,
    headline,
...
function parseJsonEntry(entry, feed) {
  if (!entry || typeof entry !== "object") return null;

  const headline = cleanText(entry.title || entry.headline, MAX_HEADLINE_CHARS);
  if (!headline) return null;

  if (feed.production && NON_ARTICLE_HEADLINE.test(headline)) return null;

  const summarySource = entry.news_meta_description || entry.summary || entry.description || entry.content_text || entry.content || "";
  const summary = cleanText(summarySource, MAX_SUMMARY_CHARS);
  if (feed.topicFilter && !feed.topicFilter.test(`${headline} ${summary}`)) return null;

  const rawUrl = entry.permalink || entry.url || entry.link;
  const url = safeTrustedUrl(rawUrl, feed);
  if (!url) return null;

  let publishedMs = NaN;
  if (entry.publish_date !== undefined && entry.publish_date !== null && entry.publish_date !== "") {
    const num = Number(entry.publish_date);
    if (Number.isFinite(num)) {
      publishedMs = num > 1e11 ? num : num * 1000;
    } else {
      publishedMs = Date.parse(entry.publish_date);
    }
  } else {
    const dateCandidate = entry.publishedAt || entry.date_published || entry.pubDate || entry.published || entry.date || entry.formatted_publish_date;
    if (dateCandidate) {
      publishedMs = Date.parse(dateCandidate);
    }
  }
  if (!Number.isFinite(publishedMs)) return null;
  ...
  if (Array.isArray(entry.categories)) {
    categories = entry.categories.map((c) => (typeof c === "string" ? c : (c && (c.name || c.title || c.label)))).filter(Boolean).join(" ");
  } else if (entry.categories && typeof entry.categories === "object") {
    const gathered = [];
    for (const group of Object.values(entry.categories)) {
      if (Array.isArray(group)) {
        for (const c of group) {
          const name = typeof c === "string" ? c : (c && (c.name || c.title || c.label));
          if (name) gathered.push(name);
        }
      } else if (typeof group === "string") {
        gathered.push(group);
      }
    }
    categories = gathered.join(" ");
  } else if (typeof entry.category === "string") {
    categories = entry.category;
  }

  const cat = categoryFor(categories, headline, feed.defaultCategory || feed.category);
  const fullText = cleanText([entry.title, entry.headline, entry.news_meta_description, entry.summary, entry.description, entry.content_text, entry.content, categories].filter(value => typeof value === "string").join(" "), MAX_BODY_BYTES);
  if (!suitableNews({ headline, summary, cat, url, source: feed.source }, fullText)) return null;
  return {
    id: url,
    cat,
...
function parseJsonFeed(raw, feed) {
  try {
    ...
    return items.map((item) => parseJsonEntry(item, feed)).filter(Boolean);
  } catch (error) {
    return [];
  }
}

function parseFeed(raw, feed = FEEDS[0]) {
  if (typeof raw !== "string") return [];
  const trimmed = raw.trim();
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    return parseJsonFeed(trimmed, feed);
  }
  const rssItems = raw.match(/<item\b[^>]*>[\s\S]*?<\/item\s*>/gi) || [];
  const entries = rssItems.length
    ? rssItems
    : (raw.match(/<entry\b[^>]*>[\s\S]*?<\/entry\s*>/gi) || []);
...
async function fetchFeed(fetchImpl, feed) {
  ...
  const request = (async () => {
    const response = await fetchImpl(feed.url, {
      ...
      headers: { accept: "application/rss+xml, application/xml;q=0.9, text/xml;q=0.8, application/json;q=0.8" },
    });
...
function dailyCategory(item) {
  ...
  // Geography describes the story, not merely the publisher's headquarters.
  if (/\b(?:Thailand|Thai|Bangkok|Singapore|Singaporean|Malaysia|Malaysian|Indonesia|Indonesian|Philippines|Filipino|Vietnam|Vietnamese|Cambodia|Laos|Myanmar|ASEAN|Southeast Asia|Asia Pacific)\b/i.test(text)) return "regional";
  if (/sport|culture|history|heritage|art|music/i.test(item.cat) || /\b(?:athlete|sport|football|olympic|concert|museum|festival|novel|poem|poetry|piano|singer|spooky story|volunteer|community)\b/i.test(text)) return "culture";
  if (/science|tech|environment|space|nature|health/i.test(item.cat) || /\b(?:research|scientist|discovery|experiment|planet|species|invention)\b/i.test(text)) return "science";
  return null;
}

async function getDailyNews(dateText, options = {}) {
  const nowMs = resolveNow(options);
  const date = dateText || new Date(nowMs + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const start = Date.parse(`${date}T00:00:00Z`);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !Number.isFinite(start) || new Date(start).toISOString().slice(0, 10) !== date || start > nowMs + DAY_MS || start < nowMs - MAX_AGE_MS) {
    return { error: "Choose a recent real date in YYYY-MM-DD format." };
  }
  const recent = await getRecentNews(options);
  const eligible = recent.items.filter(item => Date.parse(item.publishedAt) < start + DAY_MS);
  // Lock filled slots for this process/day; allow an empty slot to recover after
  // a feed outage. ponytail: editions survive refresh, not server restarts;
  // persist edition IDs if cross-restart editorial locking becomes necessary.
  const previous = editions.get(date) || [];
  const used = new Set();
  const choices = DAILY_CATEGORIES.map(({ category, label }) => {
    let article = previous.find(slot => slot.category === category)?.article;
    if (article && (!eligible.some(item => item.id === article.id) || used.has(article.id))) article = null;
    if (!article) {
      const candidates = eligible.filter(item => !used.has(item.id) && dailyCategory(item) === category)
        .sort((a,b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt) || a.id.localeCompare(b.id));
      article = candidates[Math.floor(start / DAY_MS) % Math.min(5, candidates.length)] || null;
    }
    if (article) {
      used.add(article.id);
      // Link out for the complete article; don't mirror publisher article bodies.
      article = { ...article, summary: article.summary.split(/\s+/).slice(0, 25).join(" ") };
    }
    return { category, label, article };
  });
  editions.set(date, choices);
  if (editions.size > 7) editions.delete(editions.keys().next().value);
  return { items: choices.flatMap(slot => slot.article ? [slot.article] : []), maxAgeDays: MAX_AGE_DAYS, editionDate: date, choices };
}

const DAY_MS = 24 * 60 * 60 * 1000;
module.exports = { getRecentNews, getDailyNews, dailyCategory, parseFeed, questionFor, FEEDS, SOURCE_NOTES, DAILY_CATEGORIES };
