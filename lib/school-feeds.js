...
// IMPORTANT: <calendar-id> MUST include the full `@group.calendar.google.com`
// suffix (URL-encoded to %40group... by icsUrlFor). Google returns HTTP 404 for
// the bare ID without the suffix — a bare-ID fetch is the usual cause of a false
// "feeds are down" report; the live feeds re-verified 200 (OK) on 2026-07-03.
...
function allowLoopbackCalendarFetch() {
...
function validCustomUrl(url) {
  try {
    ...
    // Loopback HTTP exists only for local development/test fixtures. Production
    // must never turn a parent-supplied URL into a request to this server's own
    // localhost/internal services.
    return allowLoopbackCalendarFetch()
      && u.protocol === "http:"
...
// Local HTTP loopback is permitted only outside production for test/dev fixture
// servers. Every production destination must use HTTPS and resolve publicly,
// and the check repeats on every redirect hop.
...
async function assertSafeUrl(u) {
  if (isLoopbackHttpDevUrl(u)) return; // non-production test/dev allowance only
  if (u.protocol !== "https:") {
