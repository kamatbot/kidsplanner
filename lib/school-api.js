...
async function performSyncKid(familyId, kidId, { force = false, nowMs = Date.now(), fetchImpl = global.fetch } = {}) {
  ...
  // The API feed owns timetable and activity data now. Run this idempotent
  // cleanup before pause/throttle checks so old extension rows disappear on
  // startup and cannot be resurrected by an old local calendar cache.
  events.removeLegacyExtensionSchoolEvents(familyId, kidId);
