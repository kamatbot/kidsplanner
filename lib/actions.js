...
function createAction(familyId, input = {}) {
  return createActionRecord(familyId, input, true);
}

function createActionRecord(familyId, input, completedNow) {
  ...
  const action = {
    ...
    createdAt: now,
    updatedAt: now,
    completedAt: actionStatus === "done" && completedNow ? now : null,
    snoozedUntil,
  };
...
function updateAction(familyId, id, patch = {}) {
  ...
  // Editing a completed item is not another completion. Legacy done rows have
  // no reliable completion date; never infer one from their last edit.
  next.completedAt = next.status !== "done" ? null
    : action.status === "done" ? action.completedAt || null : next.updatedAt;
...
// Project canonical school homework records into the Today action queue. The
...
// accepts only explicit Moodle/API source markers — it does not infer exams or
// classify records from their titles.
function projectMoodleAssignments(familyId, homeworkItems = []) {
  ...
  for (const item of homeworkItems) {
    if (!item || !["school-portal", "school-api"].includes(item.source)) {
      skipped += 1;
      continue;
    }
    ...
    // Importing already-finished homework is not a completion now. Its source
    // has no authoritative completion timestamp, so leave that date unknown.
    const result = createActionRecord(familyId, {
      title,
      dueDate: dueDate.value,
      dueTime: dueTime.value,
      assigneeType: "kid",
...
// Remove Today projections whose API-backed homework disappeared from a
// successful outstanding-homework snapshot. This is source reconciliation,
// not a user deletion, so it intentionally does not create dismissal records.
function removeProjectedHomework(familyId, homeworkIds = []) {
  const ids = new Set((Array.isArray(homeworkIds) ? homeworkIds : []).map(cleanReference).filter(Boolean));
  if (!ids.size) return { removed: 0 };
  const list = famList(familyId);
  const kept = list.filter((action) => !(action.sourceType === "homework" && ids.has(action.sourceId)));
  const removed = list.length - kept.length;
  if (removed) {
    root().actions[familyId] = kept;
    db.persist();
  }
  return { removed };
}
...
module.exports = {
  ...
  projectSchoolAssignments: projectMoodleAssignments,
  removeProjectedHomework,
  schoolSourceId,
  isSourceDismissed,
