...
// A deleted imported action must stay dismissed even though school feed rows
// are re-derived from cache on every sync. Keep this separate from the action
// list so the action shape and the public API remain unchanged.
...
function isSourceDismissed(familyId, sourceType, sourceId) {
  return sourceType === "school" && !!sourceId && dismissedSourceIds(familyId).includes(sourceId);
}

function rememberSourceDismissal(familyId, action) {
  if (!action || action.sourceType !== "school" || !action.sourceId) return;
  const dismissed = dismissedSourceIds(familyId, true);
  if (!dismissed.includes(action.sourceId)) dismissed.push(action.sourceId);
}
