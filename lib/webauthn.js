 ...
 * registrable domain, e.g. "fametc.com") and validated against the full
 * `origin` (e.g. "https://fametc.com"). These MUST match the domain the
 ...
 *
 *   RP_ID      e.g. fametc.com          (no scheme, no port)
 *   RP_ORIGIN  e.g. https://fametc.com  (comma-separated list allowed)
...
const RP_NAME = process.env.RP_NAME || "Fam ETC";
...
// domain. So if we register on `www.fametc.com` under rpID
// `www.fametc.com`, that credential is NOT usable on the apex
// `fametc.com` (and vice-versa) — they'd be two separate passkeys. To make
