...
function calendarSection(fam, range, generatedMs, ids) {
  ...
  const items = manual.concat(school)
...
function actionSection(fam, generatedMs, ids) {
  const items = actions.listForFamily(fam.id, { statuses: ["open", "snoozed"] }).slice(0, 200).map((action) => ({
    id: action.id,
  ...
  return { ...sectionMeta("actions", generatedMs, "fametc.actions"), items };
}
...
function buildFamilyContext(familyId, options = {}) {
  ...
  if (disclosure.granted.includes("actions")) sections.actions = actionSection(fam, generatedMs, ids);
