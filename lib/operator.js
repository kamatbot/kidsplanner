...
const CONTEXT_SECTIONS = new Set(["members", "room"]);
...
function validateActor(fam, actor, { allowAgent = false } = {}) {
  ...
  if (normalized.type === "kid") {
    ...
    if (normalized.userId) {
      ...
      // A provisioned kid user must link to this exact profile. A parent id is
      // also accepted because the legacy shared-device flow can post as a kid;
      // authority still remains kid-scoped because type/kidId are preserved.
...
function sanitizeSections(sections) {
  if (sections == null) return ["members", "room"];
...
function contextForFamily(familyId, options = {}) {
  ...
  const purpose = String(options.purpose || "family-assistance").trim().slice(0, 160) || "family-assistance";
  const sections = sanitizeSections(options.sections);
  const context = {
    schemaVersion: 1,
    family: { id: fam.id, name: fam.name || "Family" },
    actor,
    purpose,
    sections,
  ...
  if (sections.includes("members")) {
    context.members = {
      parents: (fam.parentIds || []).map((userId) => ({
        type: "parent",
        userId,
  ...
  if (sections.includes("room")) {
    context.room = options.roomId ? { id: String(options.roomId).slice(0, 160) } : null;
  }
  return context;
}
...
function listCases(familyId, options = {}) {
  ...
  // Apply actor/room visibility before the requested limit. Otherwise a kid's
  // own older case could disappear behind newer sibling cases in the store's
  // family-wide result window.
...
function requestApproval(familyId, caseId, input = {}) {
  ...
  return operatorStore.requestApproval(familyId, caseId, {
