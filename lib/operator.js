...
const ALLOWED_TRANSITIONS = Object.freeze({
  ...
  researching: new Set(["waiting_for_input", "proposal_ready", "waiting_for_approval", "completed", "failed", "cancelled"]),
...
function transitionCase(familyId, caseId, nextState, input = {}) {
  ...
  // ponytail: read-only completion is reservation lookup only; add another intent
  // only with its own verification contract. This never grants write authority.
  if (current.state === "researching" && target === "completed") {
    const record = operatorStore.getCase(familyId, caseId, { includeChildren: true });
    const last = record.steps.at(-1);
    if (actor.type !== "parent" || current.roomId !== "family" || record.approvals.length
        || operatorShadow.isCaseLocked(familyId, caseId)
        || !last || last.state !== "completed"
        || !["capability.workflow.learned", "capability.workflow.reused"].includes(last.kind)
        || last.output?.verified !== true || last.output?.effect !== "read-only"
        || last.output?.intent !== "reservation-lookup") {
      throw new OperatorPolicyError("Read-only lookup completion requires verified evidence and no approval history.", "OPERATOR_INVALID_TRANSITION");
    }
  }
