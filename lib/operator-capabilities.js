...
function signingKey(connection) {
  ...
  const serverKey = datacrypto.loadKey();
  if (!serverKey) {
    throw new ActorCapabilityError("Hermes actor capability signing is unavailable.");
  }
  // The bridge bearer can be hashed by anyone who possesses it. Derive the
  // capability key from a server-only secret as well, while retaining the
  // connection hash so rotation/revocation invalidates outstanding tokens.
  return crypto.createHmac("sha256", serverKey)
    .update(`fametc-operator-actor-capability:${tokenHash}`)
    .digest();
}
