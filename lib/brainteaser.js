...
function stateFor(kidId) {
  ...
  if (!r.brainteaser[kidId]) {
    r.brainteaser[kidId] = { answered: {}, servedDate: null, servedIds: [], seenIds: [], served: [] };
    migrated = true;
  }
...
function getToday(kidId, now) {
  ...
  const wrongIds = Object.keys(s.answered).filter((qid) => s.answered[qid] && s.answered[qid].wrong);
  ...
  const rotationPool = shuffle(QUESTIONS
