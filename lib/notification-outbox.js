...
try {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE IF NOT EXISTS notification_dead_letters (
      id TEXT PRIMARY KEY, kind TEXT NOT NULL, attempts INTEGER NOT NULL,
      failed_at INTEGER NOT NULL, code TEXT NOT NULL
    );
...
function hasRow(id) {
  return sqlite
    ? !!sqlite.prepare("SELECT 1 FROM notification_outbox WHERE id = ?").get(id)
    : jsonRoot().notificationOutbox.some((row) => row.id === id);
}

function hasPending(kind, dedupeKey) {
  return !!dedupeKey && hasRow(newId(kind, dedupeKey));
}

// Retire superseded recovery work once its replacement is durably queued.
// Already-started asynchronous provider calls cannot be cancelled here.
function retire(kind, dedupeKey) {
  if (!dedupeKey) return;
  removeRow(newId(kind, dedupeKey));
  if (!sqlite) db.flushSync();
}
...
function retryRow(row, error) {
  ...
  if (attempts >= MAX_ATTEMPTS || expired) {
    console.error(`[notifications] terminal outbox failure after ${attempts} attempts`);
    if (sqlite) {
      sqlite.transaction(() => {
        sqlite.prepare("INSERT OR REPLACE INTO notification_dead_letters VALUES (?, ?, ?, ?, ?)")
          .run(row.id, row.kind, attempts, Date.now(), expired ? "expired" : "attempts_exhausted");
        sqlite.prepare("DELETE FROM notification_dead_letters WHERE id NOT IN (SELECT id FROM notification_dead_letters ORDER BY failed_at DESC LIMIT 500)").run();
        removeRow(row.id);
      })();
  ...
  const lastError = "delivery_failed"; // provider bodies can contain private endpoints/tokens
  if (sqlite) {
    sqlite.prepare(`
...
async function deliverRow(row) {
  // Another item in this batch may have retired this row since dueRows().
  if (!hasRow(row.id)) return;
  ...
  if (Date.now() - Number(row.createdAt) > MAX_AGE_MS) throw new Error("Expired notification");
  const result = await delivery[row.kind](payload);
  if (result && result.failed > 0) throw new Error("Push provider delivery failed");
}
...
async function drain() {
  ...
  try {
    const rows = dueRows();
    // One bounded batch per turn. Four slow notifications cannot starve newer
    // work indefinitely; transport deadlines actually close stalled sockets.
    for (let i = 0; i < rows.length; i += 4) {
      await Promise.all(rows.slice(i, i + 4).map(async (row) => {
        try { await deliverRow(row); removeRow(row.id); }
        catch (error) { retryRow(row, error); }
      }));
    }
  } finally {
    running = false;
...
function kick(delay = 0) {
  if (!delivery || running) return;
  const requestedAt = Date.now() + Math.max(0, Number(delay) || 0);
  // A failed delivery may have scheduled the next drain minutes from now. A
  // newly enqueued notification must move that timer forward or fresh chat
  // pushes will sit behind the unrelated retry.
  if (kickTimer) {
    if (kickAt != null && kickAt <= requestedAt) return;
...
function configure(handlers) {
  // Route modules register independently (family chat, trip chat, etc.). Merge
  // rather than replace so whichever route initializes last cannot silently
  // disable delivery kinds registered by an earlier route.
  delivery = Object.assign({}, delivery || {}, handlers || {});
  kick();
}
...
function status() {
  ...
  const deadLetters = sqlite ? sqlite.prepare("SELECT COUNT(*) AS n FROM notification_dead_letters").get().n : 0;
  return { backend, pending, running, deadLetters };
}
...
module.exports = { enqueue, configure, drain, status, closeForTest, hasPending, retire };
