...
function inferDate(raw, now = new Date()) {
  if (!raw) return null;
  ...
  return localIsoDate(new Date(year, monthIdx, day));
}

// Parses the homework list page HTML into
// [{subject, title, dueDate, setDate, completed, moodleTaskId}]
function parseHomeworkHtml(html, now = new Date()) {
  if (!html) return [];
  const items = extractDivsByClass(html, "applyhwclass");
  const results = [];
  for (const item of items) {
    const taskIdM = /(?:^|\s)data-id\s*=\s*(["'])(\d+)\1/i.exec(item.fullAttrs);
    const moodleTaskId = taskIdM ? taskIdM[2] : null;
    ...
    results.push({ subject, title, dueDate, setDate, completed, moodleTaskId });
  }
  return results;
}
...
async function fetchHomework(session, moodleUserId, now = new Date()) {
  const html = await authedGet(session, `/mod/homework/view.php?h=2&userid=${encodeURIComponent(moodleUserId)}&showcompleted=0&limit=0`);
