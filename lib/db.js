...
function doFlush() {
  ...
  let snapshot;
  try {
    snapshot = serializeForDisk();
    ensureDir();
  } catch (error) {
    // Key loading/encryption and directory preparation are synchronous. They
    // need the same retry/unwedge path as asynchronous write/rename failures.
    finishFlush({ generation, error });
    return;
  }
  ...
  fs.writeFile(tmp, snapshot, (writeErr) => {
    if (writeErr) {
      fs.unlink(tmp, () => {});
      return finishFlush({ generation, error: writeErr });
    }
