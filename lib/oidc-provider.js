...
const FAMETC_ORIGIN = "https://www.fametc.com";
const PATHODDS_CLIENT_ID = "pathodds";
const PATHODDS_REDIRECT_URI = "https://www.pathodds.com/api/auth/fametc/callback";
const SIGNING_KEY_ID = "fametc-2026-01";
...
function signingKeyFromEnv() {
  ...
  if (raw.includes("BEGIN")) {
    key = crypto.createPrivateKey(raw.replace(/\\n/g, "\n"));
  } else {
    const encoded = raw.replace(/\s+/g, "");
    const der = Buffer.from(encoded, "base64");
    if (!der.length || der.toString("base64").replace(/=+$/, "") !== encoded.replace(/=+$/, "")) {
      throw new Error("OIDC_SIGNING_PRIVATE_KEY must be PEM or a base64-encoded PKCS#8 key.");
    }
    try {
      key = crypto.createPrivateKey(der.toString("utf8"));
    } catch {
      key = crypto.createPrivateKey({ key: der, format: "der", type: "pkcs8" });
    }
  }
  if (key.asymmetricKeyType !== "ed25519") throw new Error("OIDC_SIGNING_PRIVATE_KEY must be an Ed25519 key.");
  return key;
}
...
function issuer() {
  return FAMETC_ORIGIN;
}

function clientConfig() {
  return {
    clientId: PATHODDS_CLIENT_ID,
    redirectUri: PATHODDS_REDIRECT_URI,
  };
}
