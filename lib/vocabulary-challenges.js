...
const { DAILY_WORDS, LEGACY_DAILY_WORDS } = require("./sat-words");
const { LESSONS } = require("./vocabulary-lessons");
const { WEEKLY_START, getWeeklyLearning, getWeeklyContexts } = require("./weekly-learning");
...
// Historical dates retain their original content, including existing answers.
// Cutover is fixed, not derived from process start or deployment time. The first
// 211 entries were never in the legacy daily rotation; old words return only
// after that cooldown. Already-repeated historical dates cannot be undone.
const ROTATION_START = "2026-09-13";
const ROTATION_EPOCH = Date.parse(`${ROTATION_START}T00:00:00.000Z`);
const NEW_CONTEXTS = LESSONS.map(([word, , def, first, second, incorrect]) => [
  [first, `This uses ${word.toLowerCase()} correctly: ${def}`],
  [second, `This uses ${word.toLowerCase()} correctly: ${def}`],
  [incorrect, `This contradicts the meaning of ${word.toLowerCase()}: ${def}`],
]);
const ROTATION_CONTEXTS = [...NEW_CONTEXTS, ...CONTEXTS];
...
function selectionFor(date) {
  if (date.getTime() < ROTATION_EPOCH) {
    const index = (dayOfYear(date) - 1) % LEGACY_DAILY_WORDS.length;
    return { word: LEGACY_DAILY_WORDS[index], contexts: CONTEXTS[index] };
  }
  const index = Math.floor((date.getTime() - ROTATION_EPOCH) / DAY_MS) % DAILY_WORDS.length;
  return { word: DAILY_WORDS[index], contexts: ROTATION_CONTEXTS[index] };
}

function weeklyWords(date) {
  const weekly = getWeeklyLearning(date.toISOString().slice(0, 10));
  if (weekly) return weekly.weekWords;
  const monday = mondayFor(date);
  // One transition-week exception: Sunday's daily word changes, but the
  // Sep 7–13 crossword keeps its already-issued seven words and saved answers.
  const transitionWeek = monday.toISOString().slice(0, 10) === "2026-09-07";
  return Array.from({ length: 7 }, (_, offset) => {
    const day = new Date(monday.getTime() + offset * DAY_MS);
    return transitionWeek
      ? LEGACY_DAILY_WORDS[(dayOfYear(day) - 1) % LEGACY_DAILY_WORDS.length]
      : selectionFor(day).word;
  });
}
...
function getDailyVocabulary(dateText) {
  ...
  const learning = getWeeklyLearning(dateText);
  const selected = learning ? { word: learning.word, contexts: getWeeklyContexts(learning.word) } : selectionFor(date);
  const { word, contexts } = selected;
  const shift = ((Math.floor(date.getTime() / DAY_MS) % 3) + 3) % 3;
  const ordered = contexts.map(([text, explanation], index) => ({ text, explanation, incorrect: index === 2 }));
  ...
  if (learning) Object.assign(result, {
...
module.exports = { getDailyVocabulary, parseDate, dayOfYear, mondayFor, weeklyWords, ROTATION_START, WEEKLY_START };
