...
function mediaFor(meta) {
  // previewUrl/width/height are compatibility aliases for the existing native
  // ChatMedia model (which originally only knew GIF metadata). Newer clients
  // also get the explicit filename/mimeType/size/kind fields.
  const kindCode = meta.kind === "photo" ? 1 : (meta.kind === "video" ? 2 : 3);
  return {
    type: "attachment",
    attachmentId: meta.id,
    url: `/api/chat/attachments/${meta.id}`,
    previewUrl: meta.filename,
    width: meta.size,
    height: kindCode,
