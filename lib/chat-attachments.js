...
const VIDEO_TYPES = new Set(["video/mp4", "video/quicktime", "video/mpeg", "video/webm"]);
...
function magicMatches(mimeType, bytes) {
  ...
  if (mimeType === "video/webm") {
    // EBML alone includes other containers. Require the WebM DocType too.
    const head = bytes.subarray(0, 512);
    return head.subarray(0, 4).equals(Buffer.from("1a45dfa3", "hex"))
      && head.includes(Buffer.from("4282847765626d", "hex"));
  }
...
function validateMediaForScope(scopeKey, media, uploaderUserId, expectedMessageId) {
  if (!media || media.type !== "attachment" || !ID_RE.test(String(media.attachmentId || ""))) return null;
  const meta = readMeta(media.attachmentId);
  if (!meta || meta.scopeKey !== scopeKey || (meta.claimedMessageId && meta.claimedMessageId !== expectedMessageId)) return null;
...
function claimForMessage(id, scopeKey, uploaderUserId, messageId) {
  ...
  const meta = readMeta(id);
  if (!meta || meta.scopeKey !== scopeKey || meta.uploaderUserId !== String(uploaderUserId || "") || (meta.claimedMessageId && meta.claimedMessageId !== messageId)) return null;
...
// Roll back a claim only for its exact message, never an unrelated send.
function releaseClaim(id, messageId) {
  const meta = readMeta(id);
  if (!meta || meta.claimedMessageId !== messageId) return false;
  writeMeta({ ...meta, claimedMessageId: null, claimedAt: null }, datacrypto.loadKey());
  quotaIndex.unclaim(index(), id);
  return true;
}
