...
function ensureSubject(kind, familyId, principalId, opts = {}) {
  ...
  if (existingId && set.byId[existingId]) {
    ...
    // A disabled/deleted subject is never silently resurrected by a lookup.
    // If a principal is intentionally restored, that must be an explicit
    // lifecycle operation so an old PathOdds grant cannot spring back to life.
...
function findByPairwiseSubject(value, clientId = "pathodds", root = db.load()) {
  const candidate = String(value || "");
  if (!candidate) return null;
  const secret = process.env.OIDC_PAIRWISE_SUBJECT_KEY || "";
  if (String(secret).length < 32) return null;
  for (const subject of Object.values(collections(root).byId)) {
    try {
      if (derivePairwiseSubject(subject.id, clientId, secret) === candidate) return subject;
    } catch (error) {
      return null;
    }
  }
