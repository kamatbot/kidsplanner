...
let _trips = null;
// Private one-to-one Hermes rooms (lib/hermes-threads.js), scope "hermes:<userId>".
const HERMES_SCOPE_PREFIX = "hermes:";
function isHermesScope(scopeKey) {
  return typeof scopeKey === "string" && scopeKey.startsWith(HERMES_SCOPE_PREFIX);
}
function hermesThreadsLib() { return require("./hermes-threads"); }
function tripsLib() {
...
const jsonImpl = {
  insert(msg, fingerprint) {
    const thread = threadFor(msg.familyId);
    thread.messages.push(msg);
    if (fingerprint) (thread.receipts ||= {})[msg.id] = fingerprint;
    db.persist();
  },
  receipt(scopeKey, id) { return threadFor(scopeKey).receipts?.[id] || null; },
  update() {
...
const sqliteImpl = sqliteStore && {
  insert(msg, fingerprint) { sqliteStore.insert(msg, fingerprint); },
  receipt(scopeKey, id) { return sqliteStore.receipt(scopeKey, id); },
...
function migrateFromJsonToSqlite() {
  ...
  for (const familyId of Object.keys(chats)) {
    ...
    for (const msg of msgs) {
      sqliteStore.insert(Object.assign({}, msg, { familyId }), chats[familyId].receipts?.[msg.id]);
      messages++;
    }
  }
  if (messages > 0) {
...
function validateMedia(scopeKey, media, uploaderUserId, expectedMessageId) {
  if (!media || typeof media !== "object") return null;
  if (media.type === "attachment") {
    try { return chatAttachments.validateMediaForScope(scopeKey, media, uploaderUserId, expectedMessageId); } catch (e) { return null; }
  }
...
function sendMessage(scopeKey, { senderType, senderId, postedByUserId, text, card, media, buzz, clientMessageId }) {
  if (isTripScope(scopeKey)) {
    const trip = tripsLib().getTrip(scopeKey.slice(TRIP_SCOPE_PREFIX.length));
    if (!trip) return { error: "Trip not found." };
  } else if (isHermesScope(scopeKey)) {
    // A private Hermes thread exists while its owner is still in a family.
    if (!hermesThreadsLib().ownerOfScope(scopeKey)) return { error: "Thread not found." };
  } else {
  ...
  if (card?.type === "news") {
    let url;
    try { url = new URL(card.url); } catch (_) { return { error: "Invalid news article link." }; }
    if (typeof card.url !== "string" || card.url.length > 2048 || /[\u0000-\u0020\u007f]/.test(card.url)
        || url.protocol !== "https:" || url.username || url.password || !url.hostname
        || typeof card.id !== "string" || !card.id.trim() || typeof card.title !== "string" || !card.title.trim()) {
      return { error: "Invalid news article card." };
    }
    card = { type: "news", id: card.id.slice(0, 200), title: card.title.trim().slice(0, 300), url: url.href,
      source: typeof card.source === "string" ? card.source.trim().slice(0, 100) : url.hostname };
  }
  ...
  // Optional transport idempotency. A retry after a lost response returns the
  // same row, including attachment claims, rather than creating a second post.
  // The id and receipt are bound to the server-derived actor AND room.
  if (clientMessageId !== undefined && (typeof clientMessageId !== "string"
      || !/^[A-Za-z0-9_-]{16,80}$/.test(clientMessageId) || !actingUserId)) {
    return { error: "Invalid message retry key.", status: 400 };
  }
  const fingerprint = clientMessageId ? crypto.createHash("sha256").update(JSON.stringify([
    body, card || null, media?.type === "attachment" ? { type: "attachment", attachmentId: media.attachmentId } : media || null, isBuzz,
  ])).digest("hex") : null;
  const messageId = clientMessageId ? "m_" + crypto.createHash("sha256")
    .update(JSON.stringify([scopeKey, actingUserId, senderType, senderId, clientMessageId])).digest("hex").slice(0, 18) : msgId();
  if (clientMessageId) {
    const prior = impl.getMessage(scopeKey, messageId);
    if (prior) {
      if (impl.receipt(scopeKey, messageId) !== fingerprint) return { error: "That retry key belongs to a different message.", status: 409 };
      return { message: prior, existing: true };
    }
  }
  let validMedia = isBuzz ? null : validateMedia(scopeKey, media, actingUserId, clientMessageId ? messageId : undefined);
  // Never silently discard a failed attachment and send only its caption.
  if (media?.type === "attachment" && !validMedia) return { error: "That attachment is unavailable or already sent.", status: 409 };
  if (validMedia && validMedia.type === "attachment") {
    try {
      ...
      return { error: "Couldn't attach that file. Please retry.", status: 503 };
    }
    if (!validMedia) return { error: "That attachment is already sent.", status: 409 };
  }
  ...
  const msg = {
    ...
    text: body,
    card: card || null, // homework/event references or a validated news article link
  ...
  try { impl.insert(msg, fingerprint); }
  catch (error) {
    if (validMedia?.type === "attachment") {
      try { chatAttachments.releaseClaim(validMedia.attachmentId, messageId); } catch (_) { /* deterministic retry can recover the claim */ }
    }
    throw error;
  }
...
// Replace a message's card in place (a Hermes nudge recording which button
// was pressed). Text, sender and ordering never change.
function updateCard(scopeKey, messageId, card) {
  const msg = impl.getMessage(scopeKey, messageId);
