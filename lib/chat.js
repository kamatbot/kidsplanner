...
// a trip scope.
function sendMessage(scopeKey, { senderType, senderId, postedByUserId, text, card, media }) {
  ...
  const st = isTripScope(scopeKey) ? "member" : (senderType === "kid" ? "kid" : "parent");
  const msg = {
    id: msgId(),
    familyId: scopeKey,
    senderType: st,
    senderId,
    postedByUserId: postedByUserId || (isTripScope(scopeKey) ? senderId : (senderType === "parent" ? senderId : null)),
    text: body,
