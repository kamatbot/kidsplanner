...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS message_receipts (message_id TEXT PRIMARY KEY, fingerprint TEXT NOT NULL);
`);
...
const insertTransaction = db.transaction((msg, fingerprint) => {
  insertStmt.run({
    id: msg.id,
  ...
  if (fingerprint) db.prepare("INSERT INTO message_receipts VALUES (?, ?)").run(msg.id, encJson(fingerprint));
});
function insert(msg, fingerprint) { insertTransaction(msg, fingerprint); }
function receipt(familyId, id) {
  const value = db.prepare("SELECT r.fingerprint FROM message_receipts r JOIN messages m ON m.id = r.message_id WHERE m.family_id = ? AND m.id = ?").get(familyId, id)?.fingerprint;
  return value ? decJson(value) : null;
}
...
module.exports = { insert, receipt, update, get, list, listAfterId, isEmpty, _checkpointForTest, DB_FILE: dataFile("chat.sqlite"), STORE_PAGE_CAP };
