...
// Meals (docs/MEALS-PLAN.md §2/§3): "how much do I cook?" answered by a
// portion factor per member — no weight, no age, no sex, no per-kid nutrition
// target. Lives on the kid record here and on user.data.profile for parents
// (lib/routes/meals.js sets the latter via store.updateData).
const PORTIONS = new Set(["small", "regular", "big"]);
const MAX_ALLERGIES = 12;
const ALLERGY_MAX_LEN = 40;

function sanitizePortion(portion, fallback) {
  const p = String(portion || "").trim();
  return PORTIONS.has(p) ? p : fallback;
}
...
function sanitizeAllergies(allergies) {
  ...
  return allergies
    ...
    .map((a) => String(a || "").trim().slice(0, ALLERGY_MAX_LEN))
    .filter(Boolean);
}
...
// only: name, grade, color, plus Meals' portion/allergies (docs/MEALS-PLAN.md
// §3) — no email, no login, no weight/age (see brief).
function addKid(familyId, requestingParentId, { name, grade, color, portion, allergies }) {
  ...
  if (color != null && color !== '' && (typeof color !== 'string' || !/^#[0-9a-f]{6}$/i.test(color))) return { error: "Choose a valid profile color." };
  ...
  const kid = {
    ...
    color: assignedColor,
    portion: sanitizePortion(portion, "small"), // default SMALL for kids (§2)
...
// Small inline JPEG thumbnails travel with the existing family profile and cache.
function validKidPhoto(photo) {
  if (photo === '') return true;
  if (typeof photo !== 'string' || photo.length > 90000 || !/^data:image\/jpeg;base64,[A-Za-z0-9+/]+={0,2}$/.test(photo)) return false;
  const bytes = Buffer.from(photo.split(',')[1], 'base64');
  if (bytes[0] !== 0xff || bytes[1] !== 0xd8 || bytes.at(-2) !== 0xff || bytes.at(-1) !== 0xd9) return false;
  // Inspect the JPEG frame header without decoding untrusted image pixels.
...
function updateKid(familyId, requestingParentId, kidIdToEdit, patch) {
  ...
  if (patch.color != null && (typeof patch.color !== 'string' || !/^#[0-9a-f]{6}$/i.test(patch.color))) return { error: "Choose a valid profile color." };
  if (patch.photo != null && !validKidPhoto(patch.photo)) return { error: "Choose a JPEG profile picture up to 256 pixels and 65 KB." };
  ...
  if (patch.photo != null) kid.photo = patch.photo;
  if (patch.portion != null) kid.portion = sanitizePortion(patch.portion, kid.portion || "small");
  if (patch.allergies != null) kid.allergies = sanitizeAllergies(patch.allergies);
  db.persist();
  return { family: fam, kid };
}
...
function removeKid(familyId, requestingParentId, kidIdToRemove) {
  ...
  const rewards = root().fams;
  if (rewards && Object.hasOwn(rewards, familyId) && Object.hasOwn(rewards[familyId], kidIdToRemove)) {
    delete rewards[familyId][kidIdToRemove];
  }
  db.persist();
  return { family: fam };
}
...
// Removing a parent also rotates the family invite. Otherwise the removed
// person can immediately reclaim the newly-open parent slot with the same
// bearer-style code they already knew, making removal ineffective.
function removeMember(familyId, requestingParentId, memberUserId) {
  ...
  fam.inviteCode = uniqueInviteCode();
  db.persist();
  return { family: fam };
}
...
// Resolve the family a KID user belongs to (via user.data.kid linkage), but do
// not trust that linkage by itself. A removed kid can leave a stale in-memory
// user object during an in-flight request; fail closed unless the referenced
// kid profile is still a current member of the referenced family.
function familyForKidUser(kidUser) {
  const link = kidUser && kidUser.data && kidUser.data.kid;
  if (!link || !link.familyId || !link.kidId) return null;
  const fam = getFamily(link.familyId);
  if (!fam || !fam.kids.some((kid) => kid.id === link.kidId)) return null;
  return fam;
}
