...
const PATHODDS_SERVICE_BASE_URL = "https://www.pathodds.com";

function baseUrl() {
  return PATHODDS_SERVICE_BASE_URL;
}
...
function connectSubject(pairwiseSubject, role, timeZone) {
  return request("POST", "/api/auth/fametc/integration/subjects/connect", {
...
function revokeSubject(pairwiseSubject) {
  return request("POST", "/api/auth/fametc/integration/subjects/revoke", { subject: pairwiseSubject });
}
...
module.exports = { connectSubject, revokeSubject, getToday, createLaunch, signServiceRequest, baseUrl };
