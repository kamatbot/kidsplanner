...
function createAPNsClient(config = {}) {
  ...
  const timeoutMs = Math.max(100, Math.min(30000, Number(config.timeoutMs) || 10000));
  ...
  function requestOnce(deviceToken, body, opts, forceFreshToken) {
    return new Promise((resolve) => {
      ...
      timer = setTimeout(() => finish({ ok: false, status: 0, apnsId: null, reason: "RequestTimeout", shouldPruneToken: false }), timeoutMs);
      try { session = http2.connect(`https://${host}:${PORT}`); }
      catch (error) { return finish({ ok: false, status: 0, reason: "ConnectionError", shouldPruneToken: false }); }
      session.on("error", (err) => {
        finish({ ok: false, status: 0, apnsId: null, reason: "ConnectionError:" + err.message, shouldPruneToken: false });
      });
      ...
      const headers = {
        ...
        "apns-topic": opts.topic || bundleId,
      ...
      let req;
      try { req = session.request(headers); } catch (error) {
        return finish({ ok: false, status: 0, apnsId: null, reason: "RequestError", shouldPruneToken: false });
      }
      let responseHeaders = {};
      let data = "";
      req.on("response", (h) => { responseHeaders = h; });
      req.setEncoding("utf8");
      req.on("data", (chunk) => { if (data.length < 4096) data += chunk.slice(0, 4096 - data.length); });
      req.on("end", () => {
        const status = Number(responseHeaders[":status"]) || 0;
        const apnsId = responseHeaders["apns-id"] || null;
        if (status === 200) {
          finish({ ok: true, status, apnsId, reason: null, shouldPruneToken: false });
          return;
        }
        let reason = null;
      ...
      req.on("error", (err) => {
        finish({ ok: false, status: 0, apnsId: null, reason: "RequestError:" + err.message, shouldPruneToken: false });
      });
   ...
   */
  async function send({ deviceToken, payload, pushType, priority, collapseId, expiration, topic }) {
    ...
    while (true) {
      const result = await requestOnce(deviceToken, payload, { pushType, priority, collapseId, expiration, topic }, forceFreshToken);
