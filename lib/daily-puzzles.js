...
const weeklyChallenges = require("./weekly-challenges");
...
function sudokuFor(date, dateText) {
  ...
  return {
    date: dateText, available: true, type: "sudoku", title: "Daily Sudoku",
...
function getDailyPuzzle(dateText, options) {
  ...
  if (date.getTime() >= Date.UTC(2026, 8, 21) && options?.schedule !== "legacy") {
    const scheduled = weeklyChallenges.getScheduledChallenge(date, newsItems, { weeklyWords, tryBuild });
    if (day === 3) return { ...sudokuFor(date, dateText), title: scheduled.title, mentalMath: scheduled.mentalMath };
    return { date: dateText, ...scheduled };
  }
  const index = dateIndex(date);
  if ([1, 3, 5].includes(day)) {
    return sudokuFor(date, dateText);
  }
  {
