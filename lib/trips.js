...
const IMPORT_TIME_RE = /^(?:[01]\d|2[0-3]):[0-5]\d$/;
...
// `resolveName` (optional) enriches the preview with the inviter's display
// name and member faces for the join landing page. The preview is served
// UNAUTHENTICATED (anyone holding the link can join anyway), so it exposes
// no ids — initials + palette names only, and never an email.
function previewByCode(code, resolveName) {
  ...
  return {
    trip: {
      ...
      inviterName: named(trip.ownerUserId) || null,
      memberFaces: memberFaces(trip, resolveName).map((f) => ({ initial: f.initial, color: f.color })),
    },
  };
}
...
function normalizedItineraryText(value) {
  return String(value == null ? "" : value).normalize("NFKC").trim().replace(/\s+/g, " ").toLowerCase();
}

// Import duplicate identity is deliberately narrower than an itinerary row's
// full stored object: same date/time/title is a duplicate, while two different
// activities at the same time remain valid co-traveling plans.
function itineraryDuplicateKey(item = {}) {
  return [item.date || "", item.time || "", normalizedItineraryText(item.title)].join("|");
}

function getItineraryItemsBySource(tripId_, sourceType, sourceId) {
  const trip = getTrip(tripId_);
  if (!trip || !sourceType || !sourceId) return [];
  return (Array.isArray(trip.itinerary) ? trip.itinerary : []).filter((item) => (
    item.source === "hermes" && item.sourceType === sourceType && item.sourceId === sourceId
  ));
}

function findItineraryDuplicates(tripId_, proposedItems) {
  const trip = getTrip(tripId_);
  if (!trip || !Array.isArray(proposedItems)) return [];
  const existingByKey = new Map();
  for (const item of (Array.isArray(trip.itinerary) ? trip.itinerary : [])) {
    const key = itineraryDuplicateKey(item);
    if (!existingByKey.has(key)) existingByKey.set(key, item);
  }
  return proposedItems
    .map((item) => {
      const existingItem = existingByKey.get(itineraryDuplicateKey(item));
      return existingItem ? { item, existingItem } : null;
    })
    .filter(Boolean);
}
...
// Batch append used by Hermes confirmation. Every row is normalized and
// validated before the live itinerary or activity feed is touched, then one
// db.persist() snapshots the whole append. The ordinary single-item helper
// above keeps its existing per-action behavior for manual edits.
function addItineraryItems(tripId_, userId, proposedItems, options = {}) {
  const trip = getTrip(tripId_);
  if (!trip) return { error: "Trip not found." };
  if (!Array.isArray(proposedItems) || !proposedItems.length) return { items: [] };
  if (proposedItems.length > 50) return { error: "The itinerary import is too large." };

  const isHermes = options.source === "hermes";
  if (isHermes && (options.sourceType !== "chat"
      || typeof options.sourceId !== "string"
      || !CHAT_SOURCE_ID_RE.test(options.sourceId))) {
    return { error: "A valid Hermes chat source is required." };
  }

  const startDate = String(trip.startDate || "");
  const endDate = String(trip.endDate || "");
  if (!validStoredDate(startDate) || !validStoredDate(endDate) || endDate < startDate) {
  ...
  for (const proposed of proposedItems) {
    const date = String(proposed && proposed.date || "").trim();
    const time = String(proposed && proposed.time || "").trim();
    const title = String(proposed && proposed.title || "").trim().slice(0, 200);
    const category = proposed && proposed.category;
    const note = String(proposed && proposed.note || "").trim().slice(0, 1000);
    if (!validStoredDate(date) || date < startDate || date > endDate) return { error: "Every itinerary date must fall within the Trip date range." };
    if (time && !IMPORT_TIME_RE.test(time)) return { error: "Time must be in HH:MM format." };
    if (!title) return { error: "Title is required." };
    if (!CATEGORIES.has(category)) return { error: "Category must be one of food, sight, activity, transit, stay." };
    const normalized = { date, time, title, category, note };
    const key = itineraryDuplicateKey(normalized);
    if (proposedKeys.has(key)) return { error: "The itinerary import contains duplicate activities." };
  ...
  for (const item of trip.itinerary) {
    if (!nextOrder.has(item.date)) nextOrder.set(item.date, 0);
    nextOrder.set(item.date, Math.max(nextOrder.get(item.date), Number(item.order) >= 0 ? Number(item.order) + 1 : 0));
  }

  const createdAt = new Date().toISOString();
  const items = validated.map((value) => {
    const order = nextOrder.get(value.date) || 0;
    nextOrder.set(value.date, order + 1);
    const item = {
      id: itiId(),
      date: value.date,
      time: value.time,
    ...
    if (isHermes) {
      item.source = "hermes";
      item.sourceType = options.sourceType;
      item.sourceId = options.sourceId;
    }
    return item;
  });

  trip.itinerary.push(...items);
  logActivity(trip, userId, options.activityText || `imported ${items.length} itinerary item${items.length === 1 ? "" : "s"} from Hermes`);
  db.persist();
  return { items };
}

function addHermesItineraryItems(tripId_, userId, proposedItems, sourceId) {
  return addItineraryItems(tripId_, userId, proposedItems, {
    source: "hermes",
    sourceType: "chat",
    sourceId,
  });
...
module.exports = {
  ...
  addItineraryItem,
  addItineraryItems,
  addHermesItineraryItems,
  getItineraryItemsBySource,
  findItineraryDuplicates,
  itineraryDuplicateKey,
  getItineraryItem,
  updateItineraryItem,
  moveItineraryItem,
  removeItineraryItem,
