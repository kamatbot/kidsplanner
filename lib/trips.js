 ...
 * Phase A only — chat + notifications + calendar merge are Phase B).
 *
 ...
 *
 * Notifications (lib/fam-notifications.js notifyTripEvent) are Phase B — every
 * mutation that plan lists as a notification trigger (member joined,
 * itinerary add, flight/lodging add) logs to trip.activity here and leaves a
 * `// Phase B: notifyTripEvent` marker where the fan-out will be wired in.
 */
...
function joinByCode(code, userId) {
  ...
  // Phase B: notifyTripEvent(trip, userId, "joined the trip")
  db.persist();
  return { trip };
}
...
function addItineraryItem(tripId_, userId, { date, time, title, category, note } = {}) {
  ...
  if (!DATE_RE.test(d)) return { error: "A valid date (YYYY-MM-DD) is required." };
  ...
  // Phase B: notifyTripEvent(trip, userId, `added an itinerary item: ${t}`)
  db.persist();
  return { item };
}
...
// Drag-reorder: moves the item onto `date`, inserted immediately before
// `beforeId` (or appended when null/not found on that day). Reindexes order
// on the destination day, and on the source day too if it changed.
...
function addFlight(tripId_, userId, { airline, flightNo, confirmation, from, to, departs, arrives, travelerUserIds } = {}) {
  ...
  // Phase B: notifyTripEvent(trip, userId, `added a flight`)
  db.persist();
  return { flight };
}
...
function addLodging(tripId_, userId, { name, address, confirmation, checkIn, checkOut, note } = {}) {
  ...
  // Phase B: notifyTripEvent(trip, userId, `added lodging: ${n}`)
  db.persist();
  return { lodging };
}
