...
function addNote(familyId, { authorType, authorId, date, body, source, ref } = {}) {
  ...
  // News saves also post to family chat. A lost response must not create a
  // second note, award, or chat post when either existing client retries.
  // Other journal sources deliberately retain their existing save semantics.
  const cleanRef = sanitizeRef(ref);
  const id = src === "news" ? "nt_" + crypto.createHash("sha256").update(JSON.stringify([
    familyId, authorType, authorId, d, b, cleanRef?.kind, cleanRef?.id, decField(cleanRef?.context),
  ])).digest("hex").slice(0, 24) : noteId();
  const prior = src === "news" && getByIdRaw(familyId, id);
  if (prior) return { note: decorate(prior), existing: true };
  ...
  require("./fams").awardNewsComment(familyId, { ...note, body: b });
  db.persist();
